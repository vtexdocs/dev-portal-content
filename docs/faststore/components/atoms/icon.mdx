---
title: "Icon"
excerpt: Icons are simplified graphics used to represent concepts and help users navigate the UI.
sidebar_custom_props:
  image: https://vtexhelp.vtexassets.com/assets/docs/src/Icon___9437da8db13517090534a38f4870d702.png
components:
  - Icon.tsx
---

Icons are simplified graphics used to represent concepts and help users navigate the UI.

This project has a [set of icons](https://developers.vtex.com/docs/guides/faststore/reference-icons) that uses SVGs from [Phosphor icons](https://phosphoricons.com).

## Overview

<OverviewSection>
  <Icon name="ShoppingCart" weight="thin" width={32} height={32} />
  <Icon name="ShoppingCart" weight="light" />
  <Icon name="ShoppingCart" weight="regular" width={16} height={16} />
  <Icon name="ShoppingCart" weight="bold" width={20} height={20} />
</OverviewSection>

```tsx
  <Icon name="ShoppingCart" weight="thin" width={32} height={32} />
  <Icon name="ShoppingCart" weight="light" />
  <Icon name="ShoppingCart" weight="regular" width={16} height={16} />
  <Icon name="ShoppingCart" weight="bold" width={20} height={20} />
```

> ⚠️ The default values for `width` and `height` are `24`.

---

## Import

Import the component from [@faststore/ui](https://developers.vtex.com/docs/guides/faststore/components-index)

```tsx
import { Icon } from '@faststore/ui'
```

### Import Styles into your FastStore project

To apply the styles of this component in your FastStore project, import the following into your stylesheet:

```scss
@import '@faststore/ui/src/components/atoms/Icon/styles.scss';
```

Follow the instructions in the [Importing FastStore UI component styles](https://developers.vtex.com/docs/guides/faststore/using-themes-importing-ui-components-styles) tutorial.

---

## Usage

<OverviewSection>
  <Icon name="ShoppingCart" />
</OverviewSection>
```tsx
<Icon name="ShoppingCart" />
```

---

## Props

<ComponentPropsSection component="Icon" />

List of available [icons](https://developers.vtex.com/docs/guides/faststore/reference-icons)

## Customization

For further customization, you can use the following data attributes:

`data-fs-icon`

...
```tsx 
import React from 'react' import {Badge} from '@faststore/ui'

export default function BadgeBasicUsage() {
  return <Badge variant="info">New arrival</Badge>
}
```
...
Default size is `small`. Use `size="big"` for a larger badge.

...
The `Checkbox` component provides accessible markup, design tokens for customization, and states for `checked`, `disabled`, and partial (partially checked) states.
...
- Don’t use a `Checkbox` when only one option is possible. In that case, use a [Radio](https://developers.vtex.com/docs/guides/faststore/atoms-radio) instead.

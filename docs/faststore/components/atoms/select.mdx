---
title: "Select"
excerpt: Selects allow users to pick an option from a predefined list.
keywords:
  - UI library
  - ecommerce
  - react
sidebar_custom_props:
  image: https://vtexhelp.vtexassets.com/assets/docs/src/Select___8842f3b6522c3e8db0a887b3f216a6ab.png
components:
  - Select.tsx
---

[<i class="fa-brands fa-github"></i> Source code](https://github.com/vtex/faststore/tree/main/packages/components/src/atoms/Select)

Select components allow users to pick an option from a predefined list. The Select component should be used when there are more than five options available. Otherwise, consider using the [RadioGroup](https://developers.vtex.com/docs/guides/faststore/molecules-radio-group) component.

Typical use cases include:

- Filtering product listings (e.g., sorting by price, name, or popularity).
- Choosing shipping methods or delivery options.
- Selecting product variants (e.g., size, color, or quantity).

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection>
      <Select
        id="select-overview-default"
        options={{
          great: 'Great',
          ok: 'Ok',
          bad: 'Bad',
        }}
      />
    </OverviewSection>
  </Tab>
  <Tab>

    ```tsx
    import React from 'react'
    import { Select } from '@faststore/ui'

    export default function SelectBasicUsage() {
      return (
        <div>
          <Select
            id="select-overview-default"
            options={{
              great: 'Great',
              ok: 'Ok',
              bad: 'Bad',
            }}
          />
        </div>
      )
    }

    ```
  </Tab>
</Tabs>

---

## Usage

### Import the component

```tsx
import { Select } from '@faststore/ui'
```

### Import styles

To apply styles, include the following in your stylesheet:

```scss
@import '@faststore/ui/src/components/atoms/Select/styles.scss';
```

For details, see [Importing FastStore UI component styles](https://developers.vtex.com/docs/guides/faststore/using-themes-importing-ui-components-styles).

---

## Examples

### Variants

#### Disabled

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection>
      <Select
        id="select-disabled"
        options={{
          great: 'Great',
          ok: 'Ok',
          bad: 'Bad',
        }}
        disabled
      />
    </OverviewSection>
  </Tab>
  <Tab>
    ```tsx
    import React from 'react'
    import { Select } from '@faststore/ui'

    export default function SelectDisabled() {
      return (
        <Select
          id="select-disabled"
          options={{
            great: 'Great',
            ok: 'Ok',
            bad: 'Bad',
          }}
          disabled
        />
      )
    }
    ```
  </Tab>
</Tabs>

---

## Design tokens

<TokenTable>
  <TokenRow token="--fs-select-height" value="var(--fs-spacing-6)" />
  <TokenRow token="--fs-select-min-height" value="var(--fs-control-tap-size)" />
  <TokenRow
    token="--fs-select-padding"
    value="var(--fs-spacing-1) var(--fs-spacing-5) var(--fs-spacing-1) var(--fs-spacing-2)"
  />
  <TokenDivider />
  <TokenRow
    token="--fs-select-text-color"
    value="var(--fs-color-link)"
    isColor
  />
  <TokenDivider />
  <TokenRow token="--fs-select-border-radius" value="var(--fs-border-radius)" />
  <TokenDivider />
  <TokenRow token="--fs-select-bkg" value="transparent" />
  <TokenRow
    token="--fs-select-bkg-color-focus"
    value="var(--fs-color-primary-bkg-light)"
    isColor
  />
  <TokenRow
    token="--fs-select-bkg-color-hover"
    value="var(--fs-select-bkg-color-focus)"
    globalValue="var(--fs-color-primary-bkg-light)"
    isColor
  />
  <TokenDivider />
  <TokenRow
    token="--fs-select-transition-timing"
    value="var(--fs-transition-timing)"
  />
  <TokenRow
    token="--fs-select-transition-property"
    value="var(--fs-transition-property)"
  />
  <TokenRow
    token="--fs-select-transition-function"
    value="var(--fs-transition-function)"
  />
</TokenTable>

### Design tokens: Icon

The Select component includes a dropdown icon to indicate the interactive nature of the field and to provide a visual cue for user interaction.

<TokenTable>
  <TokenRow
    token="--fs-select-icon-color"
    value="var(--fs-color-link)"
    isColor
  />
  <TokenRow
    token="--fs-select-icon-position-right"
    value="var(--fs-spacing-2)"
  />
  <TokenRow token="--fs-select-icon-width" value="var(--fs-spacing-3)" />
  <TokenRow
    token="--fs-select-icon-height"
    value="var(--fs-select-icon-width)"
  />
</TokenTable>

### Design tokens: Disabled

Disabled selects use reduced opacity and alternative text colors to communicate that the field is not currently interactive.

<TokenTable>
  <TokenRow
    token="--fs-select-disabled-text-color"
    value="var(--fs-color-disabled-text)"
    isColor
  />
  <TokenRow token="--fs-select-disabled-text-opacity" value="1" />
</TokenTable>

---

## Data attributes

You can target and override `Select` styles using the following data attributes:

`data-fs-select`

`data-fs-select-icon`

---

## Props

<ComponentPropsSection component="Select" />

---

## Best Practices

### ✅ Do's

- Consider using a [`RadioGroup`](https://developers.vtex.com/docs/guides/faststore/molecules-radio-group) when few options are available. Doing so will prevent users from opening a dropdown box just to scan how many and which options are available.
- Consider using an open text [`Input`](https://developers.vtex.com/docs/guides/faststore/atoms-input) when several options are available and if the entries don’t need to be validated. Doing so will prevent users from having to read and understand all options before making a choice.
- Consider using an autocomplete field when multiple options are available and if the inputs need to be validated.
- Opt to grey out unavailable items instead of removing them completely.

---

## Related components

<SectionList>
  <SectionItem
    title="SelectField"
    description="
SelectField wraps a Select input and its corresponding Label. It allows users to select one option from a set."
    actionPath="/docs/guides/faststore/molecules-select-field"
  >
    <SelectField
      id="select-order-by"
      label="Order by:"
      options={{
        name_asc: 'Name, A-Z',
        name_desc: 'Name, Z-A',
        top_sales: 'Top Sales',
      }}
    />
  </SectionItem>
</SectionList>

---
title: "Input Field"
excerpt: InputFields are text fields used to get user inputs.
sidebar_custom_props:
  image: https://vtexhelp.vtexassets.com/assets/docs/src/Input___77d0e7a8079b7d310e8223dd9e1994f6.png
---

Input Field wraps an [Input](/docs/guides/faststore/atoms-input) and its corresponding [Label](/docs/guides/faststore/atoms-label).

## Overview

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection direction="column">
      <InputField label="Label" id="inputfield-default" />
      <InputField
        label="Input w/ Error Message"
        id="inputfield-error"
        error="Error Message"
      />
      <InputField label="Input w/ Action" id="inputfield-action" actionable />
      <InputField
        label="Input w/ Action and Error"
        id="inputfield-action-error"
        actionable
        error="Error Message"
      />
      <InputField label="Input Disabled" id="inputfield-disabled" disabled />
    </OverviewSection>
  </Tab>
  <Tab>
    ```tsx
    <InputField label="Label" id="inputfield-default" />
    <InputField
      label="Input w/ Error Message"
      id="inputfield-error"
      error="Error Message"
    />
    <InputField label="Input w/ Action" id="inputfield-action" actionable />
    <InputField
      label="Input w/ Action and Error"
      id="inputfield-action-error"
      actionable
      error="Error Message"
    />
    <InputField label="Input Disabled" id="inputfield-disabled" disabled />
    ```
  </Tab>
</Tabs>

---

## Import

Import the component from [@faststore/ui](/docs/guides/faststore/components-index)

```tsx
import { InputField } from '@faststore/ui'
```

Import Styles

```tsx
import '@faststore/ui/src/components/molecules/InputField/styles.scss'
```

---

## Usage

<OverviewSection>
  <InputField label="Label" id="inputfield-label" />
</OverviewSection>

```tsx
<InputField label="Label" id="inputfield-label" />
```

---

## Props

export const propsInputField = [
  {
    name: 'testId',
    type: 'string',
    excerpt:
      'ID to find this component in testing tools (e.g.: cypress, testing library, and jest).',
    default: 'fs-input-field'
  },
  {
    name: 'id',
    type: 'string',
    excerpt: 'ID to identify input and corresponding label.',
    required: true
  },
  {
    name: 'label',
    type: 'string',
    excerpt: 'The text displayed to identify input text.',
    required: true
  },
  {
    name: 'error',
    type: 'string',
    excerpt: 'The error message is displayed when an error occurs.'
  },
  {
    name: 'inputRef',
    type: 'MutableRefObject<HTMLInputElement | null>',
    excerpt: "Component's ref."
  },
  {
    name: 'disabled',
    type: 'boolean',
    excerpt: 'Specifies that the whole input component should be disabled.'
  },
  {
    name: 'actionable',
    type: "boolean",
    excerpt: 'Adds a Button to the component.'
  },
  {
    name: 'onSubmit',
    type: "() => void",
    excerpt: 'Callback function when button is clicked. Required for actionable input.*'
  },
  {
    name: 'onClear',
    type: "() => void",
    excerpt: 'Callback function when clear button is clicked. Required for actionable input.*'
  },
  {
    name: 'buttonActionText',
    type: "string",
    excerpt: 'The text displayed on the Button. Suggestion: maximum 9 characters.',
    default: 'Apply'
  },
  {
    name: 'displayClearButton',
    type: "boolean",
    excerpt: 'Boolean that controls the clear button.'
  },
]

<ComponentPropsSection propsList={propsInputField} />

---

## Design Tokens

<TokenTable>
  <TokenRow
    token="--fs-input-field-padding"
    value="var(--fs-spacing-2) var(--fs-spacing-2) 0"
  />
  <TokenRow
    token="--fs-input-field-color"
    value="var(--fs-color-text)"
    isColor
  />
  <TokenRow token="--fs-input-field-size" value="var(--fs-text-size-body)" />
  <TokenRow
    token="--fs-input-field-border-color"
    value="var(--fs-border-color)"
    isColor
  />
  <TokenDivider />
  <TokenRow
    token="--fs-input-field-transition-function"
    value="var(--fs-transition-function)"
  />
  <TokenRow
    token="--fs-input-field-transition-property"
    value="var(--fs-transition-property)"
  />
  <TokenRow
    token="--fs-input-field-transition-timing"
    value="var(--fs-transition-timing)"
  />
</TokenTable>

### Nested Elements

#### Label

<TokenTable>
  <TokenRow token="--fs-input-label-padding" value="0 var(--fs-spacing-2)" />
  <TokenRow
    token="--fs-input-label-color"
    value="var(--fs-color-text-light)"
    isColor
  />
  <TokenRow token="--fs-input-label-size" value="var(--fs-text-size-tiny)" />
</TokenTable>

### Variants

#### Actionable

<OverviewSection>
  <InputField label="Input w/ Action" id="inputfield-action" actionable />
</OverviewSection>
```tsx
<InputField label="Input w/ Action" id="inputfield-action" actionable />
```

#### Actionable w/ Error

<OverviewSection>
  <InputField
    label="Input w/ Error Message"
    id="inputfield-error"
    error="Error Message"
  />
</OverviewSection>
```tsx
<InputField
  label="Input w/ Error Message"
  id="inputfield-error"
  error="Error Message"
/>
```

#### Error

<OverviewSection>
  <InputField
    label="Input w/ Action and Error"
    id="inputfield-action-error"
    actionable
    error="Error Message"
  />
</OverviewSection>
```tsx
<InputField
  label="Input w/ Action and Error"
  id="inputfield-action-error"
  actionable
  error="Error Message"
/>
```

<TokenTable>
   <TokenRow
    token="--fs-input-error-message-margin-top"
    value="var(--fs-spacing-0)"
  />
  <TokenRow
    token="--fs-input-error-message-size"
    value="var(--fs-text-size-legend)"
  />
  <TokenRow token="--fs-input-error-message-line-height" value="1.1" />
  <TokenRow
    token="--fs-input-error-message-color"
    value="var(--fs-color-danger-text)"
    isColor
  />

  <TokenDivider />
  <TokenRow
    token="--fs-input-error-border-color"
    value="var(--fs-color-danger-border)"
    isColor
  />
  <TokenRow
    token="--fs-input-error-box-shadow"
    value="0 0 0 var(--fs-border-width) var(--fs-input-error-border-color)"
  />
  <TokenRow
    token="--fs-input-error-focus-ring"
    value="var(--fs-color-focus-ring-danger)"
    isColor
  />
</TokenTable>

#### Disabled

<OverviewSection>
  <InputField label="Input Disabled" id="inputfield-disabled" disabled />
</OverviewSection>
```tsx
<InputField label="Input Disabled" id="inputfield-disabled" disabled />
```

<TokenTable>
  <TokenRow
    token="--fs-input-disabled-bkg-color"
    value="var(--fs-color-disabled-bkg)"
    isColor
  />
  <TokenRow
    token="--fs-input-disabled-text-color"
    value="var(--fs-color-disabled-text)"
    isColor
  />
  <TokenRow
    token="--fs-input-disabled-border-width"
    value="var(--fs-border-width)"
  />
  <TokenRow
    token="--fs-input-disabled-border-color"
    value="var(--fs-border-color)"
    isColor
  />
</TokenTable>

---

## Customization

For further customization, you can use the following data attributes:

`data-fs-input-field`

`data-fs-input-field-error`

`data-fs-input-field-error-message`

...
To replicate this style, add `outOfStock` the prop to both `ProductCard` and `ProductCardContent`.
...
All product card-related components support all attributes supported by their respective HTML tags. Besides these attributes, the following props are also supported:

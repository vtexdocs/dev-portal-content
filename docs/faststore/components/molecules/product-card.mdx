...
The `ProductCard` displays summarized information about a product. It usually present a call-to-action button, as well as the product's name, price, and image. The `ProductCard` is composed of the following components:
...
All product card-related components support all attributes also supported by their respective HTML tags. Besides these attributes, the following props are also supported:

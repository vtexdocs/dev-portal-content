...
The `ProductCard` displays summarized information about a product. It usually present a call-to-action button, as well as the product's name, price, and image.The `ProductCard` is a compound of the following:
...
To replicate this style, add `outOfStock` prop to both `ProductCard` and `ProductCardContent`.
...
All product card-related components support the attributes also supported by their respective HTML tags. In addition to those attributes, the following props are also supported:

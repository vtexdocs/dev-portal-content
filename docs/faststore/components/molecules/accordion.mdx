...
The final Accordion component is a compound of the following:
...
All accordion-related components support all attributes also supported by the `<div>` tag, except for the `AccordionButton`, which renders the `<button>` tag and inherits [Button](/docs/faststore/atoms-button) props.
...
The `AccordionButton` component inherits [Button](/docs/faststore/atoms-button) CSS selectors.

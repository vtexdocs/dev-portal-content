---
title: "Accordion"
excerpt: Displays an expandable/collapsible list of items.
sidebar_custom_props:
  image: https://vtexhelp.vtexassets.com/assets/docs/src/Accordion___66012e5367e86c5b4422ff8d6974b8cd.png
components:
  - Accordion.tsx
  - AccordionButton.tsx
  - AccordionPanel.tsx
---

[<i class="fa-brands fa-github"></i> Source code](https://github.com/vtex/faststore/tree/main/packages/components/src/molecules/Accordion)

The `Accordion` displays an expandable/collapsible list of items. The final Accordion component is a compound of the following:- `Accordion`: wraps a series of `AccordionItem`s in a single component.

- `AccordionItem`: wraps an `AccordionButton` and an `AccordionPanel`.
- `AccordionButton`: contains the top-level items of the Accordion list. It is a clickable button that shows or hides the child items of a given `AccordionItem`.
- `AccordionPanel`: contains the child items of a given `AccordionItem`.

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection><UsageAccordion/></OverviewSection>
  </Tab>
  <Tab>
    ```tsx
    <Accordion indices={indices} onChange={onChange}>
      <AccordionItem>
        <AccordionButton>Clothing</AccordionButton>
        <AccordionPanel>
          <List>
            <li>Shorts</li>
            <li>Sweatshirt</li>
            <li>Tank tops</li>
          </List>
        </AccordionPanel>
      </AccordionItem>
      <AccordionItem>
        <AccordionButton>Bags</AccordionButton>
        <AccordionPanel>
          <List>
            <li>Backpacks</li>
            <li>Necessaire</li>
          </List>
        </AccordionPanel>
      </AccordionItem>
      <AccordionItem>
        <AccordionButton>Sale</AccordionButton>
        <AccordionPanel>
          <List>
            <li>Smartphones</li>
            <li>TVs</li>
          </List>
        </AccordionPanel>
      </AccordionItem>
    </Accordion>
    ```

  </Tab>
</Tabs>

---

## Usage

### Import the component

```tsx
import {
  Accordion,
  AccordionItem,
  AccordionButton,
  AccordionPanel,
} from '@faststore/ui'
```

### Import styles

To apply styles, include the following in your stylesheet:

```scss
@import '@faststore/ui/src/components/molecules/Accordion/styles.scss';
```

For details, see [Importing FastStore UI component styles](https://developers.vtex.com/docs/guides/faststore/using-themes-importing-ui-components-styles).

---

## Examples

### Multiple and Collapsible

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection><UsageAccordionMultiple/></OverviewSection>
  </Tab>
  <Tab>
    ```tsx
    () => {
      const [indices, setIndices] = useState(new Set([]))
      const onChange = (index) => {
        if (indices.has(index)) {
          indices.delete(index)
          setIndices(new Set(indices))
        } else {
          setIndices(new Set(indices.add(index)))
        }
      }
      return (
        <Accordion>
          <AccordionItem>
            <AccordionButton>Clothing</AccordionButton>
            <AccordionPanel>
              <List>
                <li>Shorts</li>
                <li>Sweatshirt</li>
                <li>Tank tops</li>
              </List>
            </AccordionPanel>
          </AccordionItem>
          <AccordionItem>
            <AccordionButton>Bags</AccordionButton>
            <AccordionPanel>
              <List>
                <li>Backpacks</li>
                <li>Necessaire</li>
              </List>
            </AccordionPanel>
          </AccordionItem>
          <AccordionItem>
            <AccordionButton>Sale</AccordionButton>
            <AccordionPanel>
              <List>
                <li>Smartphones</li>
                <li>TVs</li>
              </List>
            </AccordionPanel>
          </AccordionItem>
        </Accordion>
      )
    }
  ```

  </Tab>
</Tabs>

### One Collapsible at a Time

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection><UsageAccordion/></OverviewSection>
  </Tab>
  <Tab>
    ```tsx
     () => {
        const [indices, setIndices] = useState([0])
        const onChange = (index) => {
          setIndices([index])
        }
        return (
          <Accordion indices={indices} onChange={onChange}>
            <AccordionItem>
              <AccordionButton>Clothing</AccordionButton>
              <AccordionPanel>
                <List>
                  <li>Shorts</li>
                  <li>Sweatshirt</li>
                  <li>Tank tops</li>
                </List>
              </AccordionPanel>
            </AccordionItem>
            <AccordionItem>
              <AccordionButton>Bags</AccordionButton>
              <AccordionPanel>
                <List>
                  <li>Backpacks</li>
                  <li>Necessaire</li>
                </List>
              </AccordionPanel>
            </AccordionItem>
            <AccordionItem>
              <AccordionButton>Sale</AccordionButton>
              <AccordionPanel>
                <List>
                  <li>Smartphones</li>
                  <li>TVs</li>
                </List>
              </AccordionPanel>
            </AccordionItem>
          </Accordion>
        )
      }
    ```
  </Tab>
</Tabs>

---

## Design tokens

### Nested Elements

#### Item

<TokenTable>
  <TokenRow
    token="--fs-accordion-item-border-bottom-width"
    value="var(--fs-border-width)"
  />
  <TokenRow
    token="--fs-accordion-item-border-bottom-color"
    value="var(--fs-border-color-light)"
    isColor
  />
</TokenTable>

#### Button

<TokenTable>
  <TokenRow
    token="--fs-accordion-button-padding"
    value="var(--fs-spacing-3) 0"
  />
  <TokenRow
    token="--fs-accordion-button-font-size"
    value="var(--fs-text-size-3)"
  />
  <TokenRow
    token="--fs-accordion-button-font-weight"
    value="var(--fs-text-weight-bold)"
  />
  <TokenRow token="--fs-accordion-button-line-height" value="1.2" />
  <TokenRow
    token="--fs-accordion-button-color"
    value="var(--fs-color-text)"
    isColor
  />
  <TokenRow
    token="--fs-accordion-button-bkg-color"
    value="transparent"
    isColor
  />
</TokenTable>

#### Panel

<TokenTable>
  <TokenRow
    token="--fs-accordion-panel-padding-bottom"
    value="var(--fs-spacing-4)"
  />
</TokenTable>

---

## Data attributes

You can target and override `Accordion` styles using the following data attributes:

`data-fs-accordion`

`data-fs-accordion-item`

`data-fs-accordion-button`

`data-fs-accordion-panel`

The `AccordionButton` component inherits [Button](https://developers.vtex.com/docs/guides/faststore/atoms-button) CSS selectors.

---

## Props

All accordion-related components support all attributes also supported by the `<div>` tag, except for the `AccordionButton`, which renders the `<button>` tag and inherits [Button](https://developers.vtex.com/docs/guides/faststore/atoms-button) props.

Besides those attributes, the following props are also supported:

### Accordion

<ComponentPropsSection component="Accordion" />

### AccordionItem

<PropsSection propsList={propsAccordionItem} />

### AccordionButton

<ComponentPropsSection component="AccordionButton" />

### AccordionPanel

<ComponentPropsSection component="AccordionPanel" />

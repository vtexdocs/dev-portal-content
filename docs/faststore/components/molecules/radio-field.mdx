---
title: "Radio Field"
excerpt: RadioField grouped a Radio input and its corresponding Label.
sidebar_custom_props:
  image: https://vtexhelp.vtexassets.com/assets/docs/src/RadioLabel___b3fc3b7a139796e4921349df5aaa270c.png
components:
  - RadioField.tsx
---

Radio Field wraps a [Radio](https://developers.vtex.com/docs/guides/faststore/atoms-radio) input and its corresponding [Label](/componentes/atoms/label). It allows users to select one option from a set.

## Overview

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection>
      <RadioField label="Default" id="radiofield-default" />
      <RadioField label="Checked" id="radiofield-checked" checked readOnly />
      <RadioField label="Disabled" id="radiofield-disabled" disabled />
      <RadioField
        label="Checked & Disabled"
        id="radiofield-checked-disabled"
        checked
        disabled
        readOnly
      />
    </OverviewSection>
  </Tab>
  <Tab>
    ```tsx
    <RadioField label="Default" id="radiofield-default" />
    <RadioField label="Checked" id="radiofield-checked" checked />
    <RadioField label="Disabled" id="radiofield-disabled" disabled />
    <RadioField
      label="Checked & Disabled"
      id="radiofield-checked-disabled"
      checked
      disabled
    />
    ```
  </Tab>
</Tabs>

---

## Import

Import the component from [@faststore/ui](https://developers.vtex.com/docs/guides/faststore/components-index)

```tsx
import { RadioField } from '@faststore/ui'
```

### Import Styles into your FastStore project

To apply the styles of this component in your FastStore project, import the following into your stylesheet:

```scss
@import '@faststore/ui/src/components/atoms/Radio/styles.scss'
```

```scss
@import '@faststore/ui/src/components/molecules/RadioField/styles.scss';
```

Follow the instructions in the [Importing FastStore UI component styles](https://developers.vtex.com/docs/guides/faststore/using-themes-importing-ui-components-styles) tutorial.

---

## Usage

<OverviewSection>
  <RadioField label="Faststore" id="faststore" />
</OverviewSection>

```tsx
<RadioField label="Faststore" id="faststore" />
```

---

## Props

<ComponentPropsSection component="RadioField" />

---

## Design Tokens

<TokenTable>
  <TokenRow token="--fs-radio-field-gap" value="var(--fs-spacing-1)" />
</TokenTable>

---

## Customization

For further customization, you can use the following data attributes:

`data-fs-radio-field`

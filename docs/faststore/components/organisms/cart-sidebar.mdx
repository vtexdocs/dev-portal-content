...
The `CartSidebar` component displays the summary of items added to the cart along with detailed information and checkout options. It's a compound component that combines cart items, order summary, and checkout functionality. The final component is composed of the following:
...
- `CartSidebarFooter`: wraps an `OrderSummary` usually along with the Checkout Button.
...
The list section contains cart items and gifts, using padding to create comfortable spacing between items and maintain visual hierarchy.
...
- Don't make the sidebar too narrow or too wide—ensure it's comfortable for viewing cart contents.
...
<SectionList grid="column">
  <SectionItem
    title="OrderSummary"
    description="This component provides a summary of the items in the cart, including the total price, related shipping tax, and discounts."
  ...
  <SectionItem
    title="CartItem"
    description="CartItem displays the summarized information of a product inside the Cart."

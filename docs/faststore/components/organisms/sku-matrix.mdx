...
Show availability status using badges instead of numeric stock quantities by setting `availability.stockDisplaySettings` to `'showAvailability'`. The component renders an `Available` or `Out of stock` badge based on each variant's availability.

> ℹ️ The live demo below uses the default `'showStockQuantity'` setting to illustrate the matrix populated with real product data. Refer to the **Code** tab to see how to switch to the badge-based rendering.
...
<SectionList grid="column">
  <SectionItem
    title="SlideOver"
    description="The SlideOver component represents an additional section that complements the screen's information."

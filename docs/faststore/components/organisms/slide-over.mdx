---
title: "SlideOver"
excerpt: Represents an additional session that complements the screen's information.
sidebar_custom_props:
  image: https://vtexhelp.vtexassets.com/assets/docs/src/PaymentMethods___b0f344b254360daa53406cd631887adf.png
components:
  - SlideOver.tsx
  - SlideOverHeader.tsx
---

The `SlideOver` component represents an additional session that complements the screen's information.

## Import

Import the component from [@faststore/ui](https://developers.vtex.com/docs/guides/faststore/components-index)

```tsx
import { SlideOver, SlideOverHeader } from '@faststore/ui'
```

### Import Styles into your FastStore project

To apply the styles of this component in your FastStore project, import the following into your stylesheet:

```scss
@import '@faststore/ui/src/components/organisms/SlideOver/styles.scss';
```

Follow the instructions in the [Importing FastStore UI component styles](https://developers.vtex.com/docs/guides/faststore/using-themes-importing-ui-components-styles) tutorial.

---

## Usage

<Tabs items={['Example', 'Code']} defaultIndex="0">
  <Tab>
    <OverviewSection direction="column">
      <b>Open Partial</b>
      <UIProvider>
        <SlideOverPartialRight />
      </UIProvider>
      <UIProvider>
        <SlideOverPartialLeft />
      </UIProvider>
    </OverviewSection>
     <OverviewSection direction="column">
      <b>Open Full</b>
      <UIProvider>
        <SlideOverFullRight />
      </UIProvider>
      <UIProvider>
        <SlideOverFullLeft />
      </UIProvider>
    </OverviewSection>
  </Tab>
  <Tab>
    <b>Open Partial</b>

    ```tsx
      import { SlideOver } from '@faststore/ui'
      export const SlideOverPartialRight = () => {
        const { modal, openModal } = useUI()
        return (
          <>
            <Button variant="primary" onClick={openModal}>
              Right
            </Button>
            {modal && (
              <SlideOver size="partial" direction="rightSide" isOpen={modal} >
                <div style={{ padding: 16 }}>
                  <h2>SlideOver content</h2>
                  <button onClick={handleClose}>Close</button>
                </div>
              </SlideOver>
            )}
          </>
        )
      }
    ```

    ```tsx
      import { SlideOver } from '@faststore/ui'
      export const SlideOverPartialLeft = () => {
        const { modal, openModal } = useUI()
        return (
          <>
            <Button variant="primary" onClick={openModal}>
              Left
            </Button>
            {modal && (
              <SlideOver size="partial" direction="leftSide" isOpen={modal} fade={modal ? 'in' : 'out'} onDismiss={openModal} >
                <div style={{ padding: 16 }}>
                  <h2>SlideOver content</h2>
                  <button onClick={handleClose}>Close</button>
                </div>
              </SlideOver>
            )}
          </>
        )
      }
    ```

    <b>Open Full</b>
  
  ```tsx
  import { SlideOver } from '@faststore/ui'
  export const SlideOverFullRight = () => {
    const { modal, openModal } = useUI()
    return (
      <>
        <Button variant="primary" onClick={openModal}>
          Right
        </Button>
        {modal && (
          <SlideOver size="full" direction="rightSide" isOpen={modal} fade={modal ? 'in' : 'out'} onDismiss={openModal} >
            <div style={{ padding: 16 }}>
              <h2>SlideOver content</h2>
              <button onClick={handleClose}>Close</button>
            </div>
          </SlideOver>
        )}
      </>
    )
  }
  ```

    ```tsx
    import { SlideOver } from '@faststore/ui'
    export const SlideOverFullLeft = () => {
      const { modal, openModal } = useUI()
      return (
        <>
          <Button variant="primary" onClick={openModal}>
            Left
          </Button>
          {modal && (
            <SlideOverUsage size="full" direction="leftSide" isOpen={modal} fade={modal ? 'in' : 'out'} onDismiss={openModal} >
              <div style={{ padding: 16 }}>
                <h2>SlideOver content</h2>
                <button onClick={handleClose}>Close</button>
              </div>
          </SlideOver>
          )}
        </>
      )
    }
    ```

  </Tab>
</Tabs>

---

## Props

### SlideOver

<ComponentPropsSection component="SlideOver" />

### SlideOverHeader

<ComponentPropsSection component="SlideOverHeader" />

---

## Design Tokens

<TokenTable>
  <TokenRow
    token="--fs-slide-over-bkg-color"
    value="var(--fs-color-body-bkg)"
    isColor
  />
  <TokenRow
    token="--fs-slide-over-transition-timing"
    value="var(--fs-transition-timing)"
  />
</TokenTable>

### Nested Elements

#### Header

<TokenTable>
  <TokenRow
    token="--fs-slide-over-header-padding"
    value="var(--fs-spacing-2) var(--fs-spacing-3) var(--fs-spacing-2)"
  />
  <TokenRow
    token="--fs-slide-over-header-bkg-color"
    value="var(--fs-color-neutral-0)"
    isColor
  />
</TokenTable>

### Variants

#### Partial

<TokenTable>
  <TokenRow
    token="--fs-slide-over-partial-gap"
    value="calc(2 * var(--fs-grid-padding))"
  />
  <TokenRow
    token="--fs-slide-over-partial-width-mobile"
    value="calc(100vw - var(--fs-slide-over-partial-gap))"
  />
  <TokenDivider />
  <TokenRow
    token="--fs-slide-over-partial-width-notebook"
    value="calc(100% / 3)"
  />
  <TokenRow
    token="--fs-slide-over-partial-max-width-notebook"
    value="calc(var(--fs-grid-breakpoint-notebook) / 3)"
  />
</TokenTable>

---

## Customization

`data-fs-slide-over`

`data-fs-slide-over-size="full" | "partial"`

`data-fs-slide-over-state="in" | "out"`

`data-fs-slide-over-header`

`data-fs-slide-over-header-icon`

---

## Related components

<SectionList grid="column">
  <SectionItem
    title="CartSidebar"
    description="The CartSidebar displays the summary of items added to the cart along with detailed information, and checkout option."
    actionPath="/docs/guides/faststore/organisms-cart-sidebar"
  >
    <UIProvider>
      <CartSidebarUsage />
    </UIProvider>
  </SectionItem>
  <SectionItem
    title="FilterSlider"
    description="The mobile Filter view that is rendered inside a SlideOver component."
    actionPath="/docs/guides/faststore/organisms-filter-slider"
  >
    <UIProvider>
      <SearchProvider onChange={() => ({})}>
        <FilterUsage onlyMobile={true} />
      </SearchProvider>
    </UIProvider>
  </SectionItem>
</SectionList>

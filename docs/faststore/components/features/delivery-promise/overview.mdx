...
Shoppers can filter available products globally using the **Filter by store** option, which displays only products available for pickup at a specific store. This filter shows all available pickup points within a 50-km radius for pickup, as configured in Checkout. The number of pickup points displayed is unlimited.
...
On PLPs or search pages, the shopper has the following options for applying filters: [Delivery Methods](#delivery-methods-filters) and [Delivery Option](#delivery-option-filters).

>⚠️ To edit the filter labels, follow the instructions on [Managing filters and badges](https://developers.vtex.com/docs/guides/faststore/delivery-promise-implementation).

#### Delivery Methods

This filter displays available products according to the shopper's location. With this option, shoppers can filter using the following filters:
...
| ----------------- | --------------- |
| **All delivery methods** | Displays products that match all active delivery method filters: **Shipping to**, **Pickup anywhere**, **Pickup at**, **Pickup nearby**. |
| **Shipping to**  |  Displays products that can be shipped to the customer's specified location. |
| **Pickup anywhere** | Displays products available for pickup at any participating store within the merchant's network, rather than restricting results to a selected pickup location or geographic proximity. This is often used by B2B stores. |
| **Pickup at** | Displays the store specified in the global pickup point filter or the one closest to the shopper's location. To change the store set in `Pickup at {storeName}`, the shopper can click on it and select a new store in the modal that opens. This filter shows all available pickup points within a 50-km radius for pickup, as configured in Checkout. The number of pickup points displayed is unlimited. |
| **Pickup nearby** | Displays products available for pickup at the shopper's nearby stores. This filter shows up to 40 pickup points within 10 km of the shopper's location. |
...
| **Express**  | Displays products eligible for fast delivery, within less than 1 hour. |
| **Standard** | Displays products available for pickup within 4–6 hours. |
| **Pickup in 1 hour** | Displays products available for pickup in 1 hour. |

#### Availability

This filter help shoppers refine product results based on whether items can actually be purchased and fulfilled by preventing from displaying products they can't immediately purchase in their preferred way and narrowing results to only what's actually in stock.
...
| ---------- | ----------- |
| `Available for shipping` or `Unavailable for shipping` | Indicates whether the product can be shipped to the shopper's postcode. |
...
>⚠️ To edit the badge labels or disable the badges, follow the instructions on [Managing filters and badges](https://developers.vtex.com/docs/guides/faststore/delivery-promise-implementation).

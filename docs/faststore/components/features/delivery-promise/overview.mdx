...
updatedAt: "2025-10-01T12:49:14.497Z"
---

This guide explains how the solution operates, covering:

* [Hooks](#hooks): Check out the hooks used to manage user location, fulfillment options, and Delivery Promise workflows.
* [Filter options](#filter-options): Learn how shoppers can filter products by delivery or pickup methods.
* [Location priority](#understanding-location-priority): Understand the order and logic used to determine the shopper's location for accurate delivery estimates.
...
Shoppers can filter available products globally using the **Filter by store**, which displays only products available for pickup at a specific store. It shows all available pickup points within a 50 km pickup radius configured in the checkout. The number of pickup points displayed is unlimited.
...
3. **Default postal code:** If the user hasn't provided a location, isn't logged in, or has no saved address, we check if the merchant has configured a default postal code for all buyers. Learn more in [Defining a default postal code](#step-4-optional-defining-a-default-postal-code).

...
3. In the [`session`](https://developers.vtex.com/docs/guides/faststore/project-structure-config-options#session) object, set the `postalCode` value based on your scenario. See the following example:
     ...
     ```js discovery.config.js mark=12
      ...
      ```
## Managing filter and badge labels
...
By default, filters and badges are already active once Delivery Promise is implemented in the store. However, you can edit their labels in the Headless CMS. To do this, follow these steps.
...
3. Go to the **Delivery Promise** section and change their labels according to your scenario

...
updatedAt: "2025-09-03T17:36:21.645Z"
---
...
>ℹ To understand how Delivery Promise operates, see the guide [Understanding Delivery Promise operation](https://developers.vtex.com/docs/guides/faststore/delivery-promise-overview). To learn more about the Delivery Promise components, see the guide [Working with Delivery Promise components](https://developers.vtex.com/docs/guides/faststore/delivery-promise-components).
...
3. In the [`session`](https://developers.vtex.com/docs/guides/faststore/project-structure-config-options#session) object, set the `postalCode` value based on your scenario. See the following example:
     ...
     ```js discovery.config.js mark=12
      ...
      ```
## Managing filters and badges

Delivery Promise [filters]() and [badges]() help shoppers narrow products by delivery/pickup availability for their location and visually indicate shipping/pickup availability for the shopper’s location when Delivery Promise data exists, respectively.

By default, they are already badges activated once the Delivery Promise is implemented in the store. However, you can edit their label. To do this follow the next sections.

>ℹ️ For more details on filters and badges, refer to the [Understanding Delivery Promise operation](https://developers.vtex.com/docs/guides/faststore/delivery-promise-components) guide.

### Editing filters labels



### Editing badges labels

are already  If you need to edit the badge label follow these steps:

1. Access the VTEX Admin and go to **Storefront > Headless CMS**.
2. Click on the **Global Sections** Content Type.
3. Go to the **Delivery Promise** section.
4. 
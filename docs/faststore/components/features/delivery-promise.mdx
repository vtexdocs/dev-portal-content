...
With Delivery Promise, shoppers can filter available products by using a [global filter](#global-filter), which affects all navigation, or a local filter directly on a [Product List Page (PLP) or search page](#plp-or-search-page-filters), affecting only the current page they're navigating.

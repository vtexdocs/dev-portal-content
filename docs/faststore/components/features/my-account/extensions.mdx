...
3. In the `src/myAccount/navigation.ts` file, import the `getMyAccountRoutes` function from `@faststore/core` and set the new route as in the following example:
    ...
    ```ts src/myAccount/navigation.ts mark=4:8
...
To add a new section to a [default My Account page](https://developers.vtex.com/docs/guides/faststore/features-my-account-overview#default-my-account-pages), follow these steps:

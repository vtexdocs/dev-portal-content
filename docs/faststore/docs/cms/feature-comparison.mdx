...
| **Schema extension** | ❌ Not supported | ✅ Supported |
...
- **API type**: [Add a description of the API types for Headless CMS and CMS]  
- **Build Triggers**: Headless CMS triggers builds via direct HTTP webhooks that may fail silently. The CMS uses an event-driven architecture, providing built-in retries and dead-letter queues for reliability.

...
<!--
Once we have the new documentation for Content Types, update the `/TBD` annotation in the link below.
-->
...
Generates a single consolidated schema file for CMS by combining component schemas, Content Type definitions, and the FastStore base schema. This command merges your custom component schemas with the default FastStore schema to create a complete schema ready for upload.
...
| --- | --- | --- |
| `--help` | `-h` | Displays help information for the command. |
| `--out=<path>` | `-o` | Specifies the output file path for the generated schema. This option is required to produce an output, and without it, the command will only display a success message but won't save or output the schema. |
...
| `--output=<path>` | `-o` | Specifies the path to the output directory where component files will be created. Default: `cms/faststore/components` |
...
| `--output=<path>` | `-o` | Specifies the path to the output directory where Content Type files will be created. Default: `cms/faststore/pages` |
| `--sections=<path>` | `-s` | Specifies the path to the `sections.json` file used to filter components by `requiredScopes`. Default: `cms/faststore/sections.json` |

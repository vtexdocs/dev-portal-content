...
> ⚠️ Events sent via the Analytics module are not directly sent to any analytics provider (e.g., Google Analytics). This configuration can be done manually with the [`useAnalyticsEvent`](LINK) hook.
...
The Analytics SDK comes with native types based on [Enhance Ecommerce](LINK). All event types are available for use and extension. Here's the list of the events the analytics module natively supports: 
...
|`page_view`|Receives the `page_title`, `page_location` and `send_page_view` params.|
|`purchase`|Receives `currency`, `transaction_id`, `value`, `affiliation`, `coupon`, `shipping`, `tax` and `items` params.|
|`refund`|Receives `currency`, `transaction_id`, `value`, `affiliation`, `coupon`, `shipping`, `tax` and `items` params.|
...
To send the events to Google Analytics, you'll need to use the [`useAnalyticsEvent`](LINK) hook. The hook must listen to events from `sendAnalyticsEvents` and add them to the `dataLayer` (for Google Tag Manager - recommended) or call the `gtag` function directly (for gtag script implementations).

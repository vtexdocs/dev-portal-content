...
**Design tokens** are parameters that affect all UI look 'n' feel and they are the **main configuration** file of a Theme. Component-specific tokens often inherit from global tokens but are named specifically to apply those tokens in component development.## Global Tokens

FastStore uses a default theme called `Brandless`, which provides structural styles for your components and pages. Please read [Brandless's page](https://developers.vtex.com/docs/theming-overview#brandless-and-the-theming-structure) to further information about the themeable architecture.

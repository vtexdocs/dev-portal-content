---
title: 'Setting up the store code for Dynamic Content'
---

## Step 1 - Mapping pages and data-fetching functions
...
4. Inside `index.ts`, add the following code:
    ...
    - The `dynamicContent` relates the slug that we will define in the [Headless CMS](https://developers.vtex.com/docs/guides/faststore/consuming-the-dynamic-content-in-custom-sections).
...
## Step 2 - Creating Dynamic Content via API fetching

After [mapping pages and data-fetching functions](#step-1-mapping-pages-and-data-fetching-functions), you need to define these functions and create the logic to fetch the data from other data sources. There are two ways for fetching data:
...
6. Create a file called `query` inside the' resolvers' folder.
7. Add the following code to it:

    ```src/graphql/thirdParty/resolvers/query.ts
...
11. Combine your resolvers and queries into the store's dynamic content in the `index.tsx` file inside the `dynamicContent` folder.
    ...
    ```src/dynamicContent/index.ts mark=1:33
...
You can render this data in your store sections once you've set up your dynamic content fetching functions and mappings. See the [Rendering the data in custom sections](#rendering-the-data-in-custom-sections) guide for more information on using this data in a new section.
...
Once you've set up your dynamic content fetching functions and mappings, you can render this data in your store sections. For more information on using this data in a new section, see the [Rendering the data in custom sections](#rendering-the-data-in-custom-sections) guide.

### Returning the data Object

For both approaches, [data fetching using Fetch API] or Data Fetching using API Extensions feature, the data-fetching function must return an object with data as the root key in the following format:

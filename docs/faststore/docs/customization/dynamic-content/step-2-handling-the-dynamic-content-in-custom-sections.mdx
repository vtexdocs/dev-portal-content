...
- Make sure your store's Headless CMS includes the following sections in addition to your existing store page sections
   - **Global Sections**: Includes the global header and footer sections, which are required for all page types.
   - **Children Section**: Acts as the placeholder for the unique content of each page (including Landing Pages), and it is a mandatory section. Your local build will fail if this section is missing.

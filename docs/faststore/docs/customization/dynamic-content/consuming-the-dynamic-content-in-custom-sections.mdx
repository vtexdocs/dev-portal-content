...
After [mapping pages and data-fetching functions](LINK) and choosing one of the methods for [creating Dynamic Content via API fetching](LINK), use the data to render it in a store section. In this step, we will create a new Hero section, placed on a landing page, to render the image from the data.

---
title: "Customizing a component"
excerpt: "Customize the styling of a component by using design tokens that align with your store's branding."
slug: components
---

Customize the styling of a component by using design tokens that align with your store's branding.In this guide, you'll learn how to customize the style of a FastStore component using its local tokens. For the sake of this tutorial, We will use the `Listing` variant from the [Price Component](/docs/guides/faststore/atoms-price).
...
> ℹ️ Remember that when changing a component's style, you can use [Global tokens](/docs/global-tokens/overview) or styling from scratch using your preferred styling tool. However, changing the value of a global token may affect different parts of your store's interface.
...
> ℹ️ Each FastStore UI component has a list of data attributes that can be used for further customization. You can find it in the component's [Customization section](/docs/guides/faststore/atoms-price#customization).

2. Now, change the color of the `Listing` variant using the `Price` [local token](/docs/guides/faststore/atoms-price#design-tokens). For this tutorial, we will use the shade of red `#cb4242`:
...
In the example below, we will change the color of the [Price](/docs/guides/faststore/atoms-price#customization) component with the `listing` variant.
But we only want to apply this change in the `ProductShelf` section. So, we can use the `data-fs-product-shelf` data-attribute to select the [ProductShelf component](/docs/guides/faststore/organisms-product-shelf#customization) in this section.
...
When [overriding a native component](/docs/guides/faststore/building-sections-overriding-components-and-props#overriding-a-native-component) and using your custom component,
you may need to style it from scratch. In this section, you'll learn how to style a custom component.

### Before you start

Make sure you have a basic understanding of [overriding native components and props](/docs/guides/faststore/building-sections-overriding-components-and-props).
...
For better understanding, we will use the example from the [Overriding Native Components and Props](/docs/guides/faststore/building-sections-overriding-components-and-props) guide.
...
To ensure visual consistency with your store's theme, we highly recommend utilizing the [global tokens](docs/global-tokens/overview) whenever possible. Also, you can take advantage of the design tokens set provided by [FastStore UI Components](/docs/guides/faststore/).
...
- [Themes](/docs/guides/faststore/themes-overview)
- [Building and Customizing sections](/docs/guides/faststore/building-sections-component-customization-overview)

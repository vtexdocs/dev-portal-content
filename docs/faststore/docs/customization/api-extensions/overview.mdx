...
While the [FastStore API](https://developers.vtex.com/docs/guides/faststore/faststore-api-overview) provides a [GraphQL schema](https://v1.faststore.dev/reference/api/queries) for ecommerce websites that caters to common use cases, 
...
> ⚠️ Please be aware that introducing additional API endpoints to your storefront can potentially affect site performance, as already covered in 
[Best practices for fetching data](#best-practices-for-fetching-data). 
...
Do not make direct requests to external APIs to avoid compromising website performance and `504` timeout errors. Instead, use the FastStore [API extension](/docs/api-extensions) to extend…... *See the code's comment*

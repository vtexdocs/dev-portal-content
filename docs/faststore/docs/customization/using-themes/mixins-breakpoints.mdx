...
> ⚠️ Modifying these breakpoints implies changes to certain global tokens. See [Grid & Layout](https://developers.vtex.com/docs/global-tokens/grid-and-layout#breakpoints) for more details.
...
Overriding the predefined mixins follows a process similar to the [customizing breakpoints steps](https://developers.vtex.com/docs/customizing/mixins-breakpoints#customizing-breakpoints). If you already have a `custom-mixin.scss` file, you can skip to *Step 3*. Otherwise, follow the steps below:

...
For this guide we'll create two sections:
- `Call to Action` Gets an specifc action from the user.
...
All sections must be available in the Headless CMS so they can be added and managed on your store's pages. To integrate your FastStore project, please refer to the [Headless CMS integration track](https://developers.vtex.com/docs/guides/faststore/headless-cms-overview).
...
For this section we will use the following component from the Faststore UI: [Button](https://developers.vtex.com/docs/guides/faststore/atoms-button), [Icon](https://developers.vtex.com/docs/guides/faststore/atoms-icon) and [SelectField](https://developers.vtex.com/docs/guides/faststore/molecules/select-field).
...
4. Repeat the steps in [Step 3: Synchronizing the new section with the Headless CMS](#step3-synchronizing-the-new-section-with-the-headless-cms) and you should see the new section but without styles as the example below:

...
> ⚠️ We highly recommend considering [overriding props](#override-native-components-props) or utilizing the [theming tools](https://developers.vtex.com/docs/guides/faststore/using-themes-overview) provided by `@faststore/core` to achieve the desired behavior. Overriding a component inside a section will compromise the existing integration developed in [`@faststore/core`](https://developers.vtex.com/docs/guides/faststore/project-structure-overview#packagejson) by losing updates for that component.
...
Make sure you have a functioning FastStore project after following the [FastStore onboarding](https://developers.vtex.com/docs/guides/faststore/1-onboarding-overview) and [setting up the project](https://developers.vtex.com/docs/guides/faststore/getting-started-2-setting-up-the-project).
...
The [`@faststore/core`](https://developers.vtex.com/docs/guides/faststore/project-structure-overview#packagejson) package provides a set of native sections, each consisting of different components with unique functionalities.
Refer to the [List of native sections and overridable components](https://developers.vtex.com/docs/guides/faststore/building-sections-list-of-native-sections) to explore available native sections.
...
All component customizations in FastStore are performed within a section and to override props, you need to choose the desired native section and component from the [List of available native sections](https://developers.vtex.com/docs/guides/faststore/building-sections-list-of-native-sections).
...
![overrides-PDP-buy-button-props](https://vtexhelp.vtexassets.com/assets/docs/src/overrides-pdp-buy-button-props___898b016bd82c908b5b24c8822be6789a.png)> ⚠️ Remember: props changed via Headless CMS overlap the changes made through [override](/docs/guides/faststore/building-sections-overriding-components-and-props#override-native-components-props). For example, if you change a prop through override and also change it using Headless CMS, the final prop's value will be the one added using CMS.
...
1. Inside the `components` folder, create a file named `CustomBuyButton.tsx`. _If you haven't created the `components` folder yet, see the [Setting up the overrides folder](/docs/guides/faststore/building-sections-overriding-components-and-props#step-1---setting-up-the-overrides-folder) step._
...
The provided code imports the [Button](/docs/guides/faststore/atoms-button) component from the [@faststore/ui](/docs/guides/faststore/) package. The code defines the `CustomBuyButton` component that displays an alert message when clicked, based on the behavior we want.
...
1. Choose a component to override from the [list of overridable components of each native section](/docs/guides/faststore/building-sections-list-of-native-sections). In this example, we are overriding the `BuyButton` component in the `ProductDetails` section.
2. Navigate to `src/components/overrides` that we've created [before](/docs/guides/faststore/building-sections-overriding-components-and-props#step-1---setting-up-the-overrides-folder).
...
> ⚠️ Note that this will only work if your custom component's props are compatible with the overriding component.
  We encourage using [FastStore UI components](/docs/guides/faststore/) to leverage existing features, basic styles, and better props compatibility.

...
These features are useful when you need a customized behavior for a specific component not available in the [FastStore native options](https://developers.vtex.com/docs/guides/faststore/building-sections-list-of-native-sections).

> ⚠️ We highly recommend considering [overriding props](#override-native-components-props) or utilizing the [theming tools](https://developers.vtex.com/docs/guides/faststore/using-themes-components) provided by `@faststore/core` to achieve the desired behavior. Overriding a component inside a section will compromise the existing integration developed in [`@faststore/core`](https://developers.vtex.com/docs/guides/faststore/project-structure-overview#faststorecore) by losing updates for that component.
...
Make sure you have a functioning FastStore project after following the [FastStore onboarding](https://developers.vtex.com/docs/guides/faststore/getting-started-overview) and [setting up the project](https://developers.vtex.com/docs/guides/faststore/getting-started-2-setting-up-the-project).
...
The [`@faststore/core`](https://developers.vtex.com/docs/guides/faststore/project-structure-overview#faststorecore) package provides a set of native sections, each consisting of different components with unique functionalities.
Refer to the [List of native sections and overridable components](https://developers.vtex.com/docs/guides/faststore/building-sections-list-of-native-sections#faststore-native-sections) to explore available native sections.
...
All component customizations in FastStore are performed within a section and to override props, you need to choose the desired native section and component from the [List of available native sections](https://developers.vtex.com/docs/guides/faststore/building-sections-list-of-native-sections#faststore-native-sections).

For this guide, we'll use the [`BuyButton`](https://developers.vtex.com/docs/guides/faststore/molecules-buy-button#props) component to make the customization.
...
> ℹ️ Change the value of the `SECTION` variable to the name of the section you want
...
![overrides-PDP-buy-button-props](https://vtexhelp.vtexassets.com/assets/docs/src/overrides-pdp-buy-button-props___898b016bd82c908b5b24c8822be6789a.png)> ⚠️ Remember: props changed via Headless CMS overlap the changes made through [override](https://developers.vtex.com/docs/guides/faststore/building-sections-overriding-components-and-props#override-native-components-props). For example, if you change a prop through override and also change it using Headless CMS, the final prop's value will be the one added using CMS.
...
For this guide we will create a custom Buy Button that once you click on it, it displays an `alert` for the customer and the a custom Product Title.
...
> ℹ️ When creating a custom component, it's important to choose a name that
...
The provided code imports the [Button](https://developers.vtex.com/docs/guides/faststore/atoms-button) component from the [@faststore/ui](https://developers.vtex.com/docs/guides/faststore/) package. The code defines the `CustomBuyButton` component that displays an alert message when clicked, based on the behavior we want.
...
2. Navigate to `src/components/overrides` that we've created [before](https://developers.vtex.com/docs/guides/faststore/building-sections-overriding-components-and-props#step-1---setting-up-the-overrides-folder).
...
By using your custom component, you may need to customize it from scratch. See [Customizing a New Component when using Override](https://developers.vtex.com/docs/guides/faststore/using-themes-components#customizing-a-new-component-when-using-override) guide for more details about styling it.

### `Product Title`

1. After [Setting up the overrides folder](#step-1-setting-up-the-overrides-folder), inside the  `components` folder, create the `CustomProductTitle.tsx` file.
...
3. In the `overrides` folder, create the `ProductDetails.tsx` file.
...
4. Run `yarn dev` in the terminal to start the development server and access the provided localhost URL in your browser.
...
> ⚠️ Note that this will only work if your custom component's props are compatible with the overriding component.
  We encourage using [FastStore UI components](https://developers.vtex.com/docs/guides/faststore/) to leverage existing features, basic styles, and better props compatibility.

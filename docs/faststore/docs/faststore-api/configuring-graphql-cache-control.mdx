...
4. Inside the `graphqlCacheControl` flag, add the `maxAge` and `staleWhileRevalidate` objects and their values:
    ...
    | Object    | Description     |
    | --------- | --------------- |
    | `maxAge: 5 * 60` | Defines how long response stays fresh in cache. <ul><li>Responses are cached for 5 minutes (300 seconds).</li><li>After 5 minutes, the cache is considered `stale`.</li><li>`maxAge: 0` - The `0` value disables caching entirely.</li></ul> |
    | `staleWhileRevalidate: 60` | Defines how long stale cache can be used while new data fetches in the background. <ul><li>After the initial 5 minutes (when `maxAge expires`), stale cached data can still be served for up to 1 extra minute (60 seconds) while the system fetches fresh data in the background.</li><li>This ensures users see slightly older data (for 1 minute maximum) instead of waiting for a full reload.</li></ul> |
...
To ensure proper caching behavior for your custom queries, they must use the `GET` method. Queries with the `Query` suffix in their names automatically use `GET`. To enable proper caching in custom queries, follow these steps:

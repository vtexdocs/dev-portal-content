...
4. Inside `graphqlCacheControl`, add the `maxAge` and `staleWhileRevalidate` properties and their values:
    ...
    | Object    | Description     |
    | --------- | --------------- |
    | `maxAge: 5 * 60` | Defines how long response stays fresh in cache. <ul><li>Responses are cached for 5 minutes (300 seconds).</li><li>After 5 minutes, the cache is considered `stale`.</li><li>`maxAge: 0` - A `0` value disables caching entirely.</li></ul> |
    | `staleWhileRevalidate: 60` | Defines how long stale cache can be used while new data is feched in the background. <ul><li>After the initial 5 minutes (when `maxAge expires`), stale cached data can still be served for up to one extra minute (60 seconds) while the system fetches fresh data in the background.</li><li>This ensures users see slightly older data (for 1 minute maximum) instead of waiting for a full reload.</li></ul> |

    This configuration creates a tiered caching system that balances immediate performance and data freshness. See in the table below for what happens over time:
    ...
    | 300s - 360s  | Stale | <ul><li>Serves old data while fetching updates.</li><li>Simultaneously fetches fresh data in background.</li></ul> |
    | After 360s  | Expired | <ul><li>Cache fully expires.</li><li>Next request triggers a fresh data fetch.</li></ul> |

5. Open a terminal and run `yarn dev` to sync the changes and [testing the cache configuration](#testing-the-cache-configuration).

...
slug: 2-setting-up-the-headless-cms
---

import { Callout, Tabs, Tab, Steps } from 'nextra-theme-docs'Now that you are familiar with the core concepts of the [Headless CMS](/docs/headless-cms-overview) and have set it up in your VTEX account, let's get back to our **FastStore** project to define our own **Content Types** and **Sections**.
...
</Tabs>3. Create the two files below inside the `faststore` folder by running the following command:
...
> ⚠️  If you have the `@vtex/cli-plugin-cms@1.0.7` installed you can use the `{builderName}` argument to syncronize a specific builder: `vtex cms:sync {builderName}`.

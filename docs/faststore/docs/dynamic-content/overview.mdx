...
> ⚠️ Dynamic Content is natively included for stores with the 3.x. `@faststore/core` package version.
...
Note that this slug is used as the key in the dynamicContent object, withou the initial `/`.

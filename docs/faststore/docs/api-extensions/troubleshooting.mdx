...
`warn Failed to format generated files. 'yarn format:generated' threw errors` | Formatting step failed after generation. | Treat as non-blocking for GraphQL generation and fix formatting separately. Artifacts are still generated and this error does not block schema/type updates.

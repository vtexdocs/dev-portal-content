...
### Ensure store page caching

To ensure proper caching behavior when navigating store pages (such as Home, PDP, and PLP), use `GET` instead of `POST` for API requests. `GET` requests are cacheable by default, while `POST` requests bypass caching.

To follow this caching behavior for store pages, suffix new queries with `Query` to default to `GET`. For example:

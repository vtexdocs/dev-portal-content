...
> ⚠️ If you already have the directory structure created in the [Extending VTEX API schemas](/docs/api-extension/extending--api-schema#step-1---preparing-the-folders-and-files), add the `thirdParty` folder inside `src/graphql`.

...
3. In the `graphql/thirdParty/typeDefs` folder, create a `contactForm.graphql` file and add the following schema definitions:
  ...
  <CH.Code show={["contactForm.graphql"]}>
...
2. In the `graphql/thirdParty/resolvers` folder, create an `index.ts` file to consolidate the resolvers:
...
<CH.Code show={["contact-form.module.scss"]}>

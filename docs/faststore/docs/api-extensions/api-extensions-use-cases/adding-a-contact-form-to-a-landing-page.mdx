...
1. In your store repository, go to the `src` folder, and create a new `graphql` subfolder:

<CH.Code show={["contactForm.graphql"]} rows={1}>
```bash
mkdir -p src/graphql/thirdParty/{resolvers,typeDefs}
```
</CH.Code>

2. Create an `index.ts` file inside the `resolvers` folder to set up the base structure for your resolver functions.
...
<CH.Code show={["contactForm.graphql"]} rows={14}>
...
1. In the `contactForm.ts` file, add the following:

<CH.Code show={["contactForm.ts"]} rows={17}>
...
- `SubmitContactFormData`: Defines the structure of the expected input data.
- `contactFormResolver`: Implements the `submitContactForm` mutation, sending the form data to the specified API endpoint and handling the response.
...
This file imports the `contactFormResolver` and combines it with other potential resolvers into a single object.
...
3. Add the following to the `ContactForm.tsx`:

<CH.Code show={["ContactForm.tsx"]} rows={27}>
...
In the `contact-form.module.scss` file, add the following:

<CH.Code show={["contact-form.module.scss"]} rows={17}>
...
This stylesheet applies specific styles to the Contact Form component, including layout and spacing adjustments.
...
<CH.Code show={["sections.json"]} rows={11}>

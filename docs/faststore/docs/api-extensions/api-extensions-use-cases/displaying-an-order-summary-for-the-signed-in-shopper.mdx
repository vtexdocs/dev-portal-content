...
This guide describes how to extend your storefront with a section that shows signed-in shoppers how many orders they have placed and when the most recent one was. You can then place that section on any page managed in the [Headless CMS](https://developers.vtex.com/docs/guides/faststore/headless-cms-overview), such as a landing page.

Since this data belongs to one specific shopper, the resolver must authenticate as that shopper rather than as the store, so each shopper only ever sees their own orders and never another shopper's.
...
- The summary is personal data, so no shopper can access another shopper's orders.

> ⚠️ This guide covers the standard B2C flow, where the signed-in shopper is the owner of the orders. B2B and telesales sessions carry additional identity, such as an organization unit, a contract, or an operator acting on a shopper's behalf, and the endpoint's scoping in those cases is outside the scope of this guide. Check the [Orders API](https://developers.vtex.com/docs/api-reference/orders-api) reference before reusing this pattern in a B2B or telesales storefront.
...
| `extend type Query` | Adds a new root-level query, `orderSummary`, to your store's existing GraphQL schema without modifying any existing query. |
| No query arguments | The shopper's identity comes from the request, not from the caller, so there is no email or user ID to pass. Such an argument would let any caller ask for someone else's summary. |
...
> ℹ️ This step uses `validateUserAuthentication`, exported by `@faststore/api` from version 4.8.0. Update `@faststore/core` before continuing if your store is on an earlier version. See [Updating your `@faststore/core` package version](https://developers.vtex.com/docs/guides/faststore/project-structure-updating-the-core-package-version).
...
- `validateUserAuthentication(ctx)` runs first and throws immediately for a signed-out visitor, so the rest of the function only ever runs for an authenticated shopper. It is exported by `@faststore/api` from version 4.8.0, and the caller receives a `401` when the token is missing, expired, or invalid.
...
5. Run `yarn cms-sync` to send the section to the CMS, then add **OrderSummary** to a page in the VTEX Admin. See [Creating a new section](https://developers.vtex.com/docs/guides/faststore/building-sections-creating-a-new-section).

> ⚠️ The `useQuery_unstable` and `useSession_unstable` hooks are exported from `@faststore/core/experimental` and their APIs may change between releases. See [Experimental exports - Hooks and Components](https://developers.vtex.com/docs/guides/faststore/developing-and-overriding-components-experimental-exports-hooks-and-components) for more information.
...
> ℹ️ The component reads `person` from `useSession_unstable` to tell whether someone is signed in. `useAuth_unstable` answers the same question, but the property it returns was renamed between core versions, while `person` is the same in both.
...
![order-summary-for-signed-in-shopper-result](https://vtexhelp.vtexassets.com/assets/docs/src/order-summary-for-signed-in-shopper___3304d34c5d379692453ab84c2177bd02.png)

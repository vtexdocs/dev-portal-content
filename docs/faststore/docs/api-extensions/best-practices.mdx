...
# Updating your `@fastore/core` package version
If you need to update the `@fastore/core` package, follow the instructions below:

1. Open your FastStore code and navigate to the `package.json` file.

2. In `dependencies` > `@faststore/core`, change the version to the following. In the example above, we will use `^3.0.0`.

...
The FastStore API marks its own private fields with the `@auth` directive, which validates the shopper's token before the resolver runs. That directive is applied to the platform schema, so do not assume it will be enforced on a field you declare. Call the check yourself, as the first statement in the resolver:

```ts src/graphql/vtex/resolvers/orderSummary.ts
import { validateUserAuthentication } from '@faststore/api'

orderSummary: async (_: never, __: never, ctx: any) => {
  await validateUserAuthentication(ctx)

  // The rest of the resolver only runs for an authenticated shopper.
}
```

`validateUserAuthentication` is exported by `@faststore/api` from version 4.8.0. It calls the VTEX ID client, throws `UnauthorizedError` when the token is missing, expired, or invalid, and `ForbiddenError` when the token is valid but the shopper may not access the resource. The GraphQL route converts both into the corresponding HTTP statuses. Throwing is preferable to returning `null`, which the storefront cannot distinguish from an empty result.

> ⚠️ Always call the VTEX ID client, whether through `validateUserAuthentication` or your own equivalent. Values read from the session are not a substitute: the session is a separate artifact with its own lifetime, so a resolver that authorizes on session values alone can keep granting access after the shopper's token is no longer valid.

<details>
<summary>On `@faststore/core` earlier than 4.8.0</summary>

`validateUserAuthentication` is not exported before 4.8.0, so the check has to be written in the store:
...
The `try`/`catch` is not optional here. For a missing or expired token, VTEX ID answers `401` and the client throws rather than resolving with an unsuccessful status, so the check would never run and the caller would receive a `500`.
...
Forwarding the whole `cookie` header is the simplest form and the one the platform itself uses. When an endpoint needs the token on its own, read it with `getAuthCookie`, exported by `@faststore/api` from version 4.8.0:

```ts
import { getAuthCookie } from '@faststore/api'

const token = getAuthCookie(ctx.headers?.cookie ?? '', ctx.account)
```

> ℹ️ On earlier versions, parse the cookie with a parser such as the `cookie` package rather than with `split('=')[1]`. Token values can contain `=`, and truncating one causes a silent `401` response. `getAuthCookie` also normalizes duplicated cookie keys before reading the value.
...
> ⚠️ Not every VTEX endpoint is scoped by the shopper. Check the endpoint's reference before relying on the cookie alone, and treat an endpoint that answers the same for every session as the third-party case below.
...
> ⚠️ Never forward the shopper's cookie, or any VTEX cookie, to a host outside VTEX. `VtexIdclientAutCookie_{account}` is a bearer credential for that shopper on your account: whoever holds it can act as that person until it expires. Sending it to a third-party domain hands over the shopper's session, and it reaches that provider's logs and proxies along the way. Forward the cookie only to VTEX endpoints.
...
<CheckboxField label="No VTEX cookie leaves VTEX" id="checkboxfield-cookie-scope" />
<details>
<summary>The shopper's cookie is forwarded only to VTEX endpoints.</summary>

`VtexIdclientAutCookie_{account}` is a bearer credential for that shopper on your account. Sending it to a host outside VTEX hands over the shopper's session and exposes it to that provider's logs and proxies.
...
> ⚠️ The last item is the one most often missed. When you forward the cookie, the service decides, so confirm that the endpoint really is scoped by the shopper. When you cannot forward it, your resolver decides, and it must filter by an identifier that came from the session.

...
> ⚠️ Values read from the session are not a substitute for this call. The session is a separate artifact with its own lifetime, so a resolver that authorizes on session values alone can keep granting access after the shopper's token is no longer valid.

> ℹ️ `validateUserAuthentication` requires `@faststore/core` 4.8.0 or later. See [Updating your `@faststore/core` package version](https://developers.vtex.com/docs/guides/faststore/project-structure-updating-the-core-package-version).
...
> ℹ️ `getAuthCookie` normalizes duplicated cookie keys and reads the value without splitting the string, which avoids truncating a token that contains `=`.
...
Call `validateUserAuthentication` as the first line of the resolver, before any data is read or written.

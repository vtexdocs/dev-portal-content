...
```mermaid
graph LR
A[Checking the Catalog for product's title] --> B{Is there a title in the Catalog?};
B -- Yes --> C[Use the title from the Catalog];
B -- No --> D[Check 'discovery.config.js' file];
D --> E[Use general SEO title];
C --> F[Display Title];
E --> F;
```
...
```mermaid
graph LR
A[Check the Catalog for the product's description] --> B{Is there a description in the Catalog?};
B -- Yes --> C[Use the description from the Catalog];
B -- No --> D[Check the PLP description template in the 'plp' property of 'discovery.config.js'];
D --> E{Is there a PLP description template?};
E -- Yes --> F[Use the PLP description template];
E -- No --> G[Use the generic description template in the 'seo' object of 'discovery.config.js'];
C --> H[Display Description];
F --> H;
G --> H;
```
...
2. Go to the `discovey.config.js` file and in the `seo` section add the following:
  ...
  > ⚠ Change the values in `titleTemplate` and `descriptionTemplate` according to your needs. The `%s` should be used where you would like the collection name to appear. 

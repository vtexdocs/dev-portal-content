...
B -- Yes --> C[Display Title using the template];
...
```mermaid
graph LR
A[Check the Catalog for the PLP's description] --> B{Is there a description in the Catalog?};
...
E -- Yes --> F[Use the PLP description template using the PLP title in it];

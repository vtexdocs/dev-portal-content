...
> ⚠️ This feature is experimental and available only for **FastStore v3**.
...
Make sure the `@faststore/cli` package from your project is updated to the latest **v3** version. To do so, open the terminal and run:

```bash
yarn add @faststore/cli@3
```

Then run `yarn dev` to sync the changes to your project.

### Understand the experimental nature of Preact

Preact is an experimental feature available only for FastStore v3 and may have limitations that could cause issues with some store customizations.

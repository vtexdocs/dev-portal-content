...
- Preact is an experimental feature in FastStore. Therefore, it has some limitations and might cause side effects in certain customizations. Before using it in the production environment, make sure to test it to ensure compatibility with your store context.

- As Preact is a lightweight alternative to React and does not support all React features, some advanced APIs or third-party libraries designed specifically for React may not work seamlessly.

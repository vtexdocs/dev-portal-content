...
For FastStore to use the `metaTagDescription` field configured per product in the Catalog on PDP pages, a feature flag must be enabled for your store account in [Intelligent Search](https://help.vtex.com/docs/tracks/overview-intelligent-search). This flag allows Intelligent Search to include `metaTagDescription` in its link requests, which FastStore then uses as the page meta description.

Without this flag enabled, FastStore will not receive the product-level `metaTagDescription` from Intelligent Search and will fall back to the product's long description or the template configured in `discovery.config.js`. To enable this feature flag for your account, open a ticket with [VTEX Support](https://help.vtex.com/support).
...
> ⚠️ For FastStore to retrieve the `metaTagDescription` from the Catalog for PDP pages, the Intelligent Search feature flag must be enabled for your account. See [Before you begin](#before-you-begin).
...
2. Go to the `discovery.config.js` file and in the `seo` section add the following:

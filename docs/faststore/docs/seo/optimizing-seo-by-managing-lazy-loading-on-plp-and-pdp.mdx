...
> ❗ Disabling lazy loading may increase initial page load time. Apply it only for critical, above-the-fold content. To help you decide, see the [How lazy loading works in FastStore](#how-lazy-loading-works-in-faststore) and [Deciding when to lazy load](#deciding-when-to-lazy-load) sections.

## How lazy loading works in FastStore

Lazy loading is a technique that defers the loading of non-critical resources, like images or entire page sections, until they're needed. 

`LazyLoadingSection` and `ViewportObserver` work together in FastStore to improve initial page load time.
...
* `LazyLoadingSection` and `ViewportObserver`: a small utility that detects when content is approaching the viewport, triggering the `LazyLoadingSection` to render the content.

These two components work together to implement lazy loading. By deferring the rendering, hydration, and resource fetching until content is near the viewport, they significantly reduce the initial page load time, which is especially helpful for users on slower devices and networks.
...
However, be cautious: disabling lazy loading for too many elements can increase initial page load time. This practice should be applied only to critical, above-the-fold content. 

### Server-Side Rendering (SSR) and lazy loading

Server-side rendering outputs critical HTML in the initial document so users and crawlers can access it without running JavaScript. Sections wrapped by lazy loading do not appear in this initial HTML; they render and hydrate only when the section approaches the viewport.

By default, lazy loading is disabled for all sections in FastStore's PDP and PLP templates. Without lazy loading, bots can access all content at once without scrolling or executing scripts, which may speed up crawling. However, they must still download and process all media files, which can quickly consume crawl budget, slow the indexing of other pages, and reduce the number of indexed pages. Larger initial payloads can also impact user experience and bot-perceived site performance.
...
Lazy loading can improve [Core Web Vitals](https://web.dev/articles/vitals), which are important SEO ranking factors. By deferring heavy resources, lazy loading helps improve:

* **Largest Contentful Paint (LCP):** By prioritizing the hero image and other above-the-fold content, the main content loads quickly.
...
Use the following questions to help decide when to lazy load:
...
* **Above the fold content:** Is it visible on the initial screen for most devices?
    * **Yes:** **Do not** lazy load.
    * **No:** **Safe** to lazy load.
* **Structured data dependency:** Does it contain data for Schema.org or other HTML-based SEO signals?
    * **Yes:** **Do not** lazy load.
    * **No:** **Safe** to lazy load.

...
In FastStore, `LazyLoadingSection` is a section-level React wrapper that delays rendering and hydration of non-critical blocks until they approach the viewport, and `ViewportObserver` is a small utility (based on the browser's `IntersectionObserver`) that detects when content enters the viewport. Together they implement lazy loading—that is, deferring render or hydration and resource fetching until content is near or in view—reducing the initial page load time, especially on slower devices and networks.
...
In FastStore, Default PDP and PLP templates have **Skip Lazy Loading** turned on for all sections. Without lazy loading, bots can access all content at once without scrolling or executing scripts, which may speed crawling. However, they must still download and process all media files, which can quickly consume crawl budget and slow the indexing of other pages and reduce the amount of indexed pages. Larger initial payloads can also impact user experience and bot-perceived site performance.
...
- Use the **Skip Lazy Loading** flag to force a section to render server-side in the initial HTML.
- Apply it to SEO-critical and above-the-fold content (e.g., PLP titles or product tiles, PDP title, price or first image).
...
* **Largest Contentful Paint (LCP):** Prioritizing the hero image and other above-the-fold content, you can ensure the main content loads quickly.
...
Default PDP and PLP templates have **Skip Lazy Loading** turned on for all sections. Without lazy loading, the bot can access all content at once without scrolling or executing scripts, potentially making crawling faster. However, it must still download and process all media files, which can quickly consume your crawl budget and slow down the indexing of other pages and reduce the amount of indexed pages. Additionally, increases in page load time can impact both bot perception of site performance and user experience.

...
2. Locate your custom section in the file and add the `skipLazyLoadingSection` property, and set it to `true`. For example, if your custom section is named `CustomSEOSection`, the configuration would look like this:
...
1. Open the terminal and run `yarn cms-sync` to sync your local changes with the Headless CMS.

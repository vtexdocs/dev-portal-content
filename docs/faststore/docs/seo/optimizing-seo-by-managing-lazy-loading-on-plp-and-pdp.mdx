...
When creating custom sections, you can add the \*\*Skip lazy loading\*\* flag to enable skipping sections. This means lazy loading is disabled for all sections in FastStore's PDP and PLP templates. Without lazy loading, bots can access all content immediately without scrolling or running scripts, which might speed up crawling. However, they still need to download and process all media files, which can quickly consume crawl budget, slow down the indexing of other pages, and reduce the number of pages indexed. Larger initial payloads can also negatively impact user experience and make the site seem slower to bots.

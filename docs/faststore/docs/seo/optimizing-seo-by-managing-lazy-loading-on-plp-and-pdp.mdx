...
Lazy loading can improve store performance and SEO, but it must be used strategically. Below, see a checklist to help you decide when to use lazy loading to balance fast page speeds with search engine crawlability.
...
Example: A clothing store's homepage has "Seasonal Offers" promotional banners and lookbooks far down the page. Lazy loading these large images ensures shoppers see top-selling categories immediately, and the heavier promotional assets only load as they scroll.
...
Make sure the `@faststore/cli` package is updated to the latest version. If you need to update it, follow the instructions on [Updating the '@faststore/cli' package version](https://developers.vtex.com/docs/guides/faststore/project-structure-updating-the-cli-package-version).

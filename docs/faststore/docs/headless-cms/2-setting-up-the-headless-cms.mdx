...
<Tabs items={["macOs and Linux", "Windows"]} defaultIndex="0">
  <Tab>```bash mkdir -p cms/faststore ```</Tab>
  <Tab>```bash mkdir cms\faststore ```</Tab>
</Tabs>
...
<Tabs items={["macOs and Linux", "Windows"]} defaultIndex="0">
  <Tab>
    ```bash touch cms/faststore/content-types.json cms/faststore/sections.json
    ```
  </Tab>
  <Tab>
    ```bash type nul > cms\faststore\content-types.json type nul >

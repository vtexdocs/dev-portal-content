...
All sections must be available in the Headless CMS to be added and managed on your store's pages. To integrate your FastStore project, please refer to the [Headless CMS integration track](https://developers.vtex.com/docs/guides/faststore/headless-cms-1-configuring-the-vtex-account).
...
1. Access the VTEX Admin using the workspace you created earlier by navigating to the following path: `https://{workspace}--{account}.myvtex.com/admin`. Make sure to replace `{workspace}` with the designated workspace name and `{account}` with your VTEX account.

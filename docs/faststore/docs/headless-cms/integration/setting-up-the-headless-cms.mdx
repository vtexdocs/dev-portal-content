...
excerpt: "Define your first Content Type and sync it with Headless CMS (legacy) to make it available for editing in VTEX Admin."
---

Define your own **Content Types** and **Sections** so editors can create and manage pages in the VTEX Admin.
...
- Synced your CMS definitions to the Headless CMS (legacy) app
- Verified that your Content Type appears in the Headless CMS interface

This establishes the foundation for content editing workflows. In the next guide, you'll learn to structure more complex Content Types and Sections.

## Before you begin

This guide assumes you are starting without existing CMS-related files. If your project already includes `content-types.json` or `sections.json`, create only what's missing. See [Step 1: Creating the CMS folder](https://developers.vtex.com/docs/guides/faststore/headless-cms-2-setting-up-the-headless-cms#step-1-creating-the-cms-folder).
...
2. Create a new folder named `cms` at the root of your FastStore project and a subfolder named `faststore` inside the `cms` folder. Run the following command to create them:
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `content-types.json` | An array of JSON objects that describes the **Content Types** available for customization in the Headless CMS (legacy) app.                                        |
| `sections.json`      | An array of JSON objects that describes the content structure of the frontend **Section** components available for customization in the Headless CMS (legacy) app. |
...
> ℹ️ Don't worry about the structure of this file for now, as we'll learn more about it later in this track.
...
Let's now sync the changes with the Headless CMS (legacy) app and see what happens.
...
4. Sync your changes in the `cms` folder with the Headless CMS (legacy) app:
...
However, no Sections will be available yet. We'll learn more about this in the following guide, [Structuring Content Types and Sections](https://developers.vtex.com/docs/guides/faststore/headless-cms-3-adding-content-types-and-sections).

## Next step

Learn to structure more complex Content Types and Sections: [Structuring Content Types and Sections](https://developers.vtex.com/docs/guides/faststore/headless-cms-3-adding-content-types-and-sections)

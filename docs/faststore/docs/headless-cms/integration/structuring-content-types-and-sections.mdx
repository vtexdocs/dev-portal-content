...
excerpt: "Structure Content Types and Sections with JSON Schema for rich CMS editing in Headless CMS (legacy)."
---
...
- Learned how to structure Content Types with configuration schema sets
- Understood how to define Sections with JSON Schema for editor forms
- Gained knowledge of widgets and UI schemas for rich content editing
- Developed the skills to create complex Content Types and Sections independently

This knowledge enables you to build custom content management workflows tailored to your store's needs.

## Step 1: Automatically syncing your changes

1. Before creating more complex definitions of Content Types and Sections, go to the root of your FastStore project and run `yarn dev` to enable the FastStore CLI watch mode.  Make sure to use a development workspace.
  > ℹ️ This command will monitor any changes in the project, including the new section you have created, and automatically merge it with the default sections available in [`faststore/core`](https://developers.vtex.com/docs/guides/faststore/getting-started-4-understanding-the-project-structure).
2. Open another terminal and run `faststore cms-sync` to automatically sync your changes in the `cms` folder with the Headless CMS (legacy) app.
3. Open the **VTEX Admin**, access **Storefront > Headless CMS** and keep it next to your code to see your changes while editing the `cms` folder. Refreshing the Admin may be necessary to see your changes.

## Step 2: Adding Content Types to Headless CMS (legacy)
...
| `name` | The name presented at the Headless CMS (legacy) app.  |
...
## Step 3: Adding Sections to Headless CMS (legacy)
...
The guide offers detailed information on the process of creating Sections and integrating them into the Headless CMS (legacy) for easy editing and management.
...
| Key           | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| :------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `title`       | The name that identifies your component in the Headless CMS (legacy) app.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
Once you're satisfied with your Content Types and Sections, promote them to production: [Promoting your changes](https://developers.vtex.com/docs/guides/faststore/headless-cms-4-promoting-your-changes)

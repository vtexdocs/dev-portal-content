...
Widgets are interface elements in the [Headless CMS](https://developers.vtex.com/docs/guides/faststore/headless-cms-overview) that merchants can use to manage store sections. These can include date pickers, dropdown menus, image uploaders, and more, helping editors create and edit content.
...
3. In the `sections.json` file, find the section where you want to add the widget. In the following example, the new section, `Custom Newsletter`, receives the [`media-gallery`](#media-gallery) widget.
...
4. Open the terminal and run `yarn cms-sync` to synchronize your changes with the Headless CMS. Once the changes are synchronized, you will see the following message: `info: CMS synced successfully...`.
5. Go to the VTEX Admin and access **Storefront > Headless CMS**.
6. Open the page where you want to add the new section. In this context, we will add the `Custom Newslet` to the `Home`.
7. Add the section to the page, and you will notice the new widgets you have added, `media-gallery`.

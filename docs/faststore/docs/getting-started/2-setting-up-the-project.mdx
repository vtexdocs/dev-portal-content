...
Now that your store is up and running after [FastStore Onboarding](/docs/faststore/1-faststore-onboarding-overview), you're ready to set up your development environment and create a unique storefront that reflects your brand. Let's get started!
...
>ℹ️ Although you might opt to use an alternative code editor, note that Visual Studio Code might help you follow along with the FastStore documentation since our documentation may include screenshots from VS Code.
...
>ℹ️ For Windowns users we highly recommend to run the terminal in administrator mode. To run in this mode, right-click on the terminal icon and select `Run as administrator`.

2. Clone the repository created during the [Onboarding](/docs/faststore/1-faststore-onboarding-onboarding#repository-and-live-store-created) by running the following command in the terminal:
```bash
...
>ℹ️ Note that your store configuration was already set during the [Onboarding](/docs/faststore/1-faststore-onboarding-onboarding#repository-and-live-store-created). Therefore, there is no need to connect your project to your VTEX account. If you want to review these configurations, you can access the `faststore.config.js` file in the root directory of your project's repository.
...
```
>ℹ️ The `dev` command is a part of the FastStore CLI, a pre-configured command line interface in your starter project. Using the CLI, this command enables you to execute code to keep your store's project up-to-date with the [@faststore/core package](/docs/faststore/getting-started-3-project-structure). For more information, refer to the topic on [FastStore CLI](/docs/faststore/getting-started-4-faststore-cli).
...
Let's make the first level of customization which is update the store's theme with [Theming](/docs/faststore/docs-brandless). For this customization we are going to use the [Soft Blue theme](/docs/faststore/themes-soft-blue) and we will make some updates on it.
...
>ℹ️ It is important to highlight that the store's logo is set through the [VTEX Headless CMS](https://www.faststore.dev/tutorials/cms/0).

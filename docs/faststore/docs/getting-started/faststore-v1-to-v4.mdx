...
hidden: true
updatedAt: "2026-06-15T19:48:17.734Z"
---

FastStore v4 is the latest version of the framework and is recommended for storefront development.

For stores still running in FastStore v1, the migration happens in two phases:
- **Phase 1:** Migrate the storefront to FastStore v3 in a new subaccount.
- **Phase 2:** Upgrade that v3 project to FastStore v4.

This guide explains the migration process to FastStore v4, how it works, and the steps required.
...
| **2. Development start** | No new development in v1. | Set up the v3 project and repository. |
...
| **Completion** | Eventually retired after migration. | Becomes the live store (go live on v3). |

## Before you begin

This guide is for VTEX accounts currently using FastStore v1 that need to begin the migration path to FastStore v4 by first moving to FastStore v3.
...
This subaccount is a dedicated, isolated environment that ensures all v3 development and deployments don't affect your live FastStore v1 store. It will have its own domain, which prevents redirect conflicts, and an independent Headless CMS, which avoids any data or content cross-contamination between environments.
...
### Step 2 - Start development for v3
...
Each VTEX account or subaccount manages its own VTEX IO app installations, permissions, and URLs. Some apps that work in FastStore v1 may not work out of the box in v3 and may require a custom integration or an alternative distribution method.
...
### Step 4 - Recreate the Headless CMS (legacy) content

Recreate all necessary pages, components, and content manually in the Headless CMS (legacy) for your v3 project. For more information, check [Working with Headless CMS (legacy)](https://developers.vtex.com/docs/guides/faststore/headless-cms-overview).

> ℹ️ Content migration between v1 and v3 Headless CMS (legacy) instances isn't supported.
...
> ℹ️ Migrating Intelligent Search settings from a v1 project to v3 isn't supported.
...
### Step 7 - Split traffic between v1 and v3

Once you've completed all previous steps, [open a ticket with VTEX Support](https://help.vtex.com/support?cultureInfo=en-us) at least five days before your planned migration date to request traffic splitting between your main account (FastStore v1) and subaccount (FastStore v3).

This gradual rollout allows you to slowly redirect users from the old store to the new one, monitor performance, and ensure stability before going fully live on v3.
...
### Step 8 - Upgrade from v3 to v4

Once the v3 storefront is stable, proceed with the FastStore v4 upgrade by following the guide [Upgrading FastStore to v4](https://developers.vtex.com/docs/guides/faststore/getting-started-upgrading-faststore-to-v4).

This second phase updates the project dependencies and configuration, includes CMS follow-up tasks, and requires setting Node.js v24 in WebOps before production rollout.

...
In the **Require password for** section, there are the following:

* **Default domains (*.vtex.app):** Applies password protection to testing URLs that use VTEX's default `vtex.app` domain. Select this option to restrict access to internal preview environments, branch previews, and other non-public links generated during development.
* **Custom domains (e.g. www.mystore.com):** Applies password protection to your public-facing store domain configured in FastStore (for example, www.mystore.com). Select this option when you want to temporarily gate access to your production site or a custom staging domain during soft launches, QA, or controlled rollouts.

  > ℹ️ Password protection for custom domains isn't enabled by default. If you want to restrict access for your store domains configured in FastStore, you must open a ticket with [VTEX Support](https://help.vtex.com/en/support).

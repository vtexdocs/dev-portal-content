...
Preview URLs that aren't accessed for a period of time are automatically deactivated to save resources. This doesn't delete your preview or its data, it only pauses it until it's needed again. When you open a deactivated preview URL, one of the following happens:
...
> ⚠️ If you run automated tests against preview URLs, that a request made right after a period of inactivity may not immediately return the storefront (status `200`), since the preview could still be waking up. Add a retry to your test setup to account for this.

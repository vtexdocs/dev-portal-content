...
FastStore WebOps allows you to customize the branch that generates production builds and configure the branches that generate preview deployments through the **Integrations** tab. This feature is particularly useful for merchants who maintain separate production and QA accounts, enabling you to manage both environments from a single repository without code duplication.
...
> ℹ️ If you don't configure a custom production branch, WebOps will use the repository's default branch (typically `main`). This maintains backward compatibility with existing projects.
...
> ℹ️ For guidance on structuring these environments across several stores, including why a workspace doesn't replace a separate testing account, see [Managing environments](https://developers.vtex.com/docs/guides/faststore/multi-store-managing-environments).
...
- **Focus on specific workflows:** Combine include and exclude patterns to create a focused preview workflow that matches the development process of your team.
...
- The default branch of your repository is `develop`, where ongoing development occurs.
...
You can use include and exclude patterns to fine-tune the branches that create preview builds by following these steps:
...
2. Next to the rule you need to update, click `⋮` and then click `Edit`.
...
1. Go to the **Rules** section.
2. Next to the rule you need to delete, click `⋮` and then click `Delete`.

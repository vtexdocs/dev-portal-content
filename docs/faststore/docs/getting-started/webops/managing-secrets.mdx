...
In this guide, you'll learn how to manage secrets in your FastStore project using [WebOps](https://developers.vtex.com/docs/guides/faststore/getting-started-2-starting-the-project).
...
VTEX has a permissions system, and only users assigned to a role with the required [License Manager resources](https://help.vtex.com/en/tutorial/license-manager-resources--3q6ztrC8YynQf6rdc6euk3) can manage secrets.
...
To manage secrets, go to your [FastStore WebOps dashboard](https://developers.vtex.com/docs/guides/faststore/1-onboarding-dashboard) and navigate to the **Settings** tab.
...
>⚠ After you create, update, or delete a secret, it may take a few minutes for the change to propagate. Wait 5 minutes before triggering a new deployment.
>
> If you deployed your storefront immediately after creating, updating, or deleting a secret and the change isn’t reflected in your store, wait a few minutes and run another deploy before opening a support ticket.
...
3. Wait **5 minutes**, then click `Redeploy` to redeploy your website with the updated secret configuration. You'll see the message `Redeploying with secret changes`, and a pop-up with `Deployment created successfully` will open.
...
The created secret will be available in the **Current Keys** section.
...
4. Wait **5 minutes**, then click `Redeploy` to redeploy your website with the updated secret configuration. You'll see the message `Redeploying with secret changes`, and a pop-up with `Deployment created successfully` will open.
...
1. Go to the `Current Keys` section.
2. Next to the secret you need to delete, click `⋮` and then click `Delete`.
...
5. Wait **5 minutes**, then click `Redeploy` to redeploy your website with the updated secret configuration. You'll see the message `Redeploying with secret changes`, and a pop-up with `Deployment created successfully` will open.

---
title: 'Requirements'
excerpt: "This guide provides the requirements for starting a FastStore project."
...
3. Check if the `vtex.admin-cms@1.x` and `vtex.admin-cms-graphql-rc` are in the list. If so, proceed to the [next step](#step-3-installing-headless-cms-plugins). Otherwise, continue following these instructions.
...
Once the app and its dependency are installed, proceed to the [next step](#step-3-installing-headless-cms-plugins).
...
1. Open the terminal and log in to your VTEX account using the VTEX IO CLI:
    ...
    > ⚠️ Remember to replace the value between curly brackets with your account name.
...
> ⚠️
>
> - **For Windows users:** If you encounter problems while installing the plugin, please refer to the [Troubleshooting](https://developers.vtex.com/docs/guides/faststore/troubleshooting-error-installing-headless-cms) article
>
> - **For macOS users:** If you encounter problems, run `yarn config set ignore-engines true` to ignore the Node.js incompatibility version, then run `vtex plugins install cms` again.
>
> The recommendation is to use the latest Node.js version or a version higher than `15.0.0`.
...
To enable the app, check which version of the [Edition Store](https://developers.vtex.com/docs/guides/vtex-io-documentation-edition-app) is installed in your account.
...
<details>
<summary><b>Accounts that want to migrate to the `5.x`</b></summary>

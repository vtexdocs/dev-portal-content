...
To prevent content duplication that could cause build failures during the project's onboarding, it is important to confirm the installation of the following apps in your account:
...
3. Check if the `vtex.admin-cms@1.x` and `vtex.admin-cms-graphql-rc` are in the list. If so, proceed to check the [next requirement](#step-3-installing-headless-cms-plugins). Otherwise, continue following these instructions.
...
Once the app and its dependency are installed, proceed to check the [next requirement](#step-3-installing-headless-cms-plugins).
...
1. Open the terminal and log in to your VTEX account using the VTEX IO CLI:
    ...
    > ⚠️ Remember to replace the values between curly brackets with your account name.
...
> ⚠️
>
> - **Windows user:** If you encounter problems while installing the plugin, please refer to the [Troubleshooting](https://developers.vtex.com/docs/guides/faststore/troubleshooting-error-installing-headless-cms) article
>
> - **macOS user:** If you encounter problems, run `yarn config set ignore-engines true` to ignore the Node.js incompatibility version, then run `vtex plugins install cms` again.
>
...
Before developing a storefront in FastStore, you need to enable the app on your account. For more information about Intelligent Search and its integration, please refer to the [VTEX Intelligent Search](https://help.vtex.com/tracks/vtex-intelligent-search) and [Integration Settings](https://help.vtex.com/en/tracks/vtex-intelligent-search--19wrbB7nEQcmwzDPl1l4Cb/6wKQgKmu2FT6084BJT7z5V) guides.
...
If your account is using the `vtex.edition-store@5.x` major version, the VTEX Intelligent Search app is already included, and you need to integrate the app via VTEX Admin.
...
After successfully installing the app, integrate it by accessing VTEX Admin, and navigating to **Store Settings > Intelligent Search > Integrations**, and click `START INTEGRATION`.

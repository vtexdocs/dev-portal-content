...
While the development server is running, changes you save are hot-reloaded in the browser. To stop the server, press `Ctrl + C` in the terminal. To restart it, run `yarn dev` again.
...
Let's make the first level of customization, which updates the store's [theme](https://developers.vtex.com/docs/guides/faststore/themes-overview#available-themes). For this customization, we will use the [Soft Blue theme](https://developers.vtex.com/docs/guides/faststore/themes-soft-blue).

...
For standard cart management in FastStore projects, the recommended approach is to use the experimental hooks:

- `useCart_unstable` – hook to access and manage the cart state.
- `cartStore_unstable` – store to directly interact with cart functions such as `addItem`.

```tsx
import { useCart_unstable as useCart, cartStore_unstable as cartStore } from '@faststore/core/experimental'
```
For the full list of available hooks, see [Experimental exports - Hooks and Components](https://developers.vtex.com/docs/guides/faststore/developing-and-customizing-components-experimental-exports-hooks-and-components).
The `@faststore/sdk` Cart module (with `CartProvider` and `createCartStore`) is intended for advanced cases where you need to implement your own custom state manager.
...
2. Implement the `validateCart` function to handle server-side validation:
    ...
    ```tsx
    import { CartProvider, Cart } from '@faststore/sdk'
    import { useCart_unstable as useCart } from '@faststore/core/experimental'
    // In the App's root component:

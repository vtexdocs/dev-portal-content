...
4. To add styles to this new section, create a `section.module.scss` file and add the following:
...
5. Create a file named `index.tsx` inside the components folder and add the following:
...
6. In your store root repository, if you don’t have it already, create a folder named `cms`.
7. Inside `cms`, create the `faststore` folder.
8. Within the `cms/faststore` folder, create the `sections.json` file.
9. In the `sections.json` file, add the new section you want to display in the Headless CMS. The schema below defines how the Headless CMS renders a section:
...
2. Go to the newsletter section on the store page and fill in the fields **Your Name** and **Your Email**.

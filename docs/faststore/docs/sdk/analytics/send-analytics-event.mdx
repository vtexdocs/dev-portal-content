...
The `sendAnalyticsEvent` function triggers events in the browser, following the [Google Analytics 4 (GA4) data model](https://developers.google.com/analytics/devguides/collection/ga4/reference/events). This function is primarily used for ecommerce tracking but also supports custom events /* remeber to add a link to 'custom events' once we have the following task done: https://vtex-dev.atlassian.net/browse/EDU-13545 */, serving as a centralized point for managing events.

...
> ⚠️ To ensure the correct functioning of the checkout for all customer access scenarios, all subdomains used by the store must be added to the host list. If the store operates without the `www.` domain, you must include both the `www.` and non-`www.` domains in the Hosts list. Likewise, if the store operates with custom prefixes (e.g., `shop.mystore.com`) both domains (`shop.mystore.com` and `mystore.com`) should be added in the list. This guarantees that the checkout process works seamlessly when customers access the store with or without `www.` or any other custom prefix.

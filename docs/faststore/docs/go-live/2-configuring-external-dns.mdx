...
- [Set up your VTEX account.](https://developers.vtex.com/docs/guides/faststore/go-live-1-setting-up-vtex-account-for-faststore)
- Develop your FastStore project with **Next.js** by following the [Getting Started track](https://developers.vtex.com/docs/guides/faststore/getting-started-overview).
...
6. Add the `secure` subdomain (e.g., `secure.mystore.com`) to the list.
   ![host-configuration](https://vtexhelp.vtexassets.com/assets/docs/src/account-settings___31213b16a82656055b5b2b2f524dfb37.png)
7. Click the **Save** button.

> ⚠️ To ensure the correct functioning of the checkout for all customer access scenarios, all subdomains used by the store must be added to the host list. If the store operates without the `www.` domain, you must include both the `www.` and non-`www.` domains in the Hosts list. This guarantees that the checkout process works seamlessly when customers access the store with or without `www.`.
...
6. Add the `SITE_HOST` key with the main domain specified in [Configuring domains in VTEX Account Settings](https://developers.vtex.com/docs/guides/faststore/go-live-2-configuring-external-dns#step-2---configuring-domains-in-vtex-account-management) as the value.

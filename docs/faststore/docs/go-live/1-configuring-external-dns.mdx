...
#### Stores hosted on Vercel

Stores hosted on [Vercel](https://vercel.com/) can benefit from a single domain. This feature simplifies domain management by routing Checkout and My Account pages through the store’s primary domain (e.g., `www.domain.com`), removing the need for a separate `secure.domain.com` subdomain.
...
> ⚠️ Stores hosted on [Vercel](https://vercel.com/), see the [Single-domain setup for stores hosted on Vercel](#step-5--single--domain-setup-only-for-stores-hosted-on-vercel) section.
...
5. Add the `SITE_HOST` key with the main domain specified in [Stores hosted on Vercel](#stores-hosted-on-vercel) as the value.

...
excerpt: 'Learn how to configure external DNS settings for your FastStore and VTEX websites to use a custom domain.'
...
7. Add the root domain as well. For example, if your domain is `www.mystore.com`, add `mystore.com`. To complete the setup, you must add three hosts for each domain:
...
In to your FastStore project, you must also configure your project to point to the correct addresses and domains.
...
To ensure your store operates efficiently, it is important to correctly configure custom domain prefixes for checkout and secure product viewing. These are the following scenarios where you need to configure custom domain prefixes:
...
To ensure customers can view products in the secured environment on your stores's final domain, add the `subDomainPrefix` property to the [`discovery.config.js`](https://developers.vtex.com/docs/guides/faststore/project-structure-overview#files) file. 

...
7. Add the root domain as well (e.g., `mystore.com`).
...
3. In the `discovery.config.js` file, update the `Production URLs` field with the new URLs, and replace the `rootDomain` value with your store root domain name. See the example below:
   ...
   > ⚠️ For stores with prefixed domains, add the `subDomainPrefix` property to `discovery.config.js` to ensure product visibility in the secured environment. For more information, see the [Adding `subDomainPrefix` for prefixed domains](https://developers.vtex.com/docs/guides/faststore/go-live-1-configuring-external-dns#adding-subdomains-to-the-host-list) section.
   >⚠️ Remove any `secureSubdomain` reference or `secure` host.

4. Save your changes.
5. Open the `vtex.env` file.
6. Add the `SITE_HOST` key with the main store domain as the value specified in [Stores hosted on Vercel in Step 3 - Configuring domains in VTEX Account Settings](https://developers.vtex.com/docs/guides/faststore/go-live-1-configuring-external-dns#step-3-configuring-domains-in-vtex-account-settings), for example, `SITE_HOST=www.yourstore.com`
7. Save your changes.
8. Open a Pull Request, commit your changes, and deploy them to `main`.

> ⚠️ FastStore WebOps domain configuration takes priority over `discovery.config.js`. Use WebOps as your single source of truth when possible.

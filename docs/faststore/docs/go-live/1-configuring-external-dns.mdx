...
> ⚠️ For stores hosted on [Vercel](https://vercel.com/), see the [Single-domain setup for stores hosted on Vercel](#step-5--single--domain-setup-only-for-stores-hosted-on-vercel) section.
...
#### Setup domains in VTEX Account Settings
...
4. In the **Hosts** field, enter your main domain (example: `www.mystore.com`) into the list.
5. Click `Add`.
6. Add the subdomain root domain as well; for example, if your domain is `www.mystore.com`, add `mystore.com`. For a complete setup, you must add three hosts for each domain:
   - `www.mystore.com`  (store domain)
   - `mystore.com` (root domain)
   - `domain.com/checkout` (checkout/myaccount subdomain)
...
5. Add the `SITE_HOST` key with the main domain specified in [Setup domains in VTEX Account Settings](#setup-domains-in-vtex-account-settings) as the value.

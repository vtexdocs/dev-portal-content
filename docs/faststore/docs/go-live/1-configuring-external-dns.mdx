...
>⚠️ Open the ticket in advance, several business days before the store goes live. VTEX Support processing typically takes about three business days, and you will have five days to set up regular DNS pointing after you receive a successful reply from VTEX Support.
...
Stores with custom prefixes, like `shop.mystore.com`, need to add both the prefix and main domain (`shop.mystore.com` and `mystore.com`). This guarantees that the checkout process works seamlessly when customers access the store with or without `www.` or any other custom prefix.

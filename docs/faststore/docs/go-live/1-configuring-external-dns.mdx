...
> ⚠️ If your strategy requires additional subdomains (e.g., `b2b.mystore.com`), add them as separate CNAMEs and ensure they are [listed under Hosts](#step-3-configuring-domains-in-vtex-account-settings).  
...
> ⚠️ Checkout and secure flows will not work if any used domain or subdomain is missing. If your store uses multiple subdomains or custom prefixes, see [For stores with custom domain prefixes](#for-stores-with-custom-domain-prefixes) section.

#### For stores with custom domain prefixes
...
- Stores with prefixed domains: Add `subDomainPrefix` for prefixed domains.

<details>
<summary><b>Adding subdomains to the host list</b></summary>
...
<details>
<summary><b>Adding `subDomainPrefix` for prefixed domains</b></summary>
...
> ℹ️ You will commit and deploy these changes later in [Step 4](#step-4-associating-your-custom-domain-with-your-faststore-project).
...
6. Add the `SITE_HOST` key with the main store domain as the value specified in [Step 3](#step-3-configuring-domains-in-vtex-account-settings), for example, `SITE_HOST=www.yourstore.com`
7. Save your changes.
...
1. Open a ticket with [VTEX Support](https://help.vtex.com/en/support) requesting a **domain migration** for your FastStore WebOps implementation. Include the following information:
   - VTEX account name
   - Store's main domain (example: `www.mystore.com`)

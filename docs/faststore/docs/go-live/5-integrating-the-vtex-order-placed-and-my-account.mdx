...
- Integrated your FastStore project with the VTEX Checkout. See [this guide](https://developers.vtex.com/docs/guides/faststore/go-live-4-integrating-the-vtex-checkout) for more information.
...
- Check the Editon Store on your VTEX account:
  ...
  > ℹ️ Run `vtex edition get` to check if the `vtex.edition-store` app is installed on your account. If not, [open a support ticket](https://help.vtex.com/en/support) communicating you need the `vtex.edition-store@5.x` to be installed on your account so you can integrate your FastStore project with the VTEX platform.

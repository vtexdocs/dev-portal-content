...
To create a complete digital commerce experience for your FastStore project, you need to provide shoppers with essential features like a Checkout and My Account space. In this guide, learn how to implement the **Order Placed** page, which displays a successful message right after a successful checkout, and a **My Account** space, which allows customers to manage their orders and personal data (e.g., profile info, password, addresses, and credit cards) on a single page.

The Order Placed and My Account pages run on VTEX's secure subdomain, separate from your FastStore domain. Customizing these pages requires using [VTEX IO](https://developers.vtex.com/docs/guides/vtex-io-documentation-what-is-vtex-io). In this guide, we'll use VTEX IO to build the necessary VTEX apps to integrate these pages with your store, but it's not required deep knowledge of VTEX IO.

## Before you begin

Before proceeding any further, make sure you already have:

- Integrated your FastStore project with the VTEX Checkout. For more information, see [Integrating the VTEX Checkout](https://developers.vtex.com/docs/guides/faststore/go-live-3-integrating-the-vtex-checkout).
- Installed the VTEX IO CLI on your machine. For more information, see [VTEX IO CLI installation and command reference](https://developers.vtex.com/vtex-developer-docs/docs/vtex-io-documentation-vtex-io-cli-installation-and-command-reference).
...
- Run `vtex edition get` to check if the `vtex.edition-store` app is installed on your account. If not, [open a support ticket](https://help.vtex.com/en/support) communicating that you need the `vtex.edition-store@5.x` to be installed on your account so you can integrate your FastStore project with the VTEX platform.

...
2. Import the hook or component into your custom component file.
   ...
   > ⚠️ Replace `{ComponentName}` with the name of the component you want to import, for example `useCartToggleButton`.
...
- [Hooks](#hooks)
  - [useCart_unstable API reference](#usecart_unstable-api-reference)
...
| `useCart_unstable`             | Accesses the current cart's status and actions. For more details on this hook's properties and functions, see [useCart_unstable - API reference](#usecart_unstable-api-reference). |
...
Use the [`Image_unstable`](https://github.com/vtex/faststore/blob/105a8b2f69ffd8438532d7c9eb959ed26a567675/packages/core/src/components/ui/Image/Image.tsx) component to optimize image performance in your storefront. This component uses [Thumbor](https://www.thumbor.org/) for image resizing, cropping, and caching the results on a Content Delivery Network (CDN). Use it whenever you need to [create a new section for the Headless CMS](https://developers.vtex.com/docs/guides/faststore/building-sections-creating-a-new-section).

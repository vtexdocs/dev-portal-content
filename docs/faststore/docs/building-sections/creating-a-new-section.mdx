...
>ℹ️ Sections are page components that wrap up other components to create a section. For example, the Hero section is a native section that includes the following components: `Hero (UIHero)`, `HeroImage (UIHeroImage)`, and `HeroHeader (UIHeroHeader)`.

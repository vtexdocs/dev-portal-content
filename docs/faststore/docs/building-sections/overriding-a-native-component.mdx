...
>⚠️ **By overriding a component, you lose all the integration developed on [@faststore/core](/docs/getting-started/3-project-structure#faststorecore) for that component.** We highly recommend trying first [overriding props](/docs/building-sections/overriding-components-props) and/or using our [theming tools](/docs/themes/overview) to achieve the desired behavior without compromising the existing integration implemented in `@faststore/core`. You can find the available props for each component on the [Components UI](/components).

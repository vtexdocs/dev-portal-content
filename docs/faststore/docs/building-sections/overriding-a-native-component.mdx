...
>⚠️ **By overriding a component, you lose all the integration developed on [@faststore/core](/docs/faststore/getting-started-3-project-structure) for that component.** We highly recommend trying first [overriding props](/docs/faststore/building-sections-overriding-components-props) and/or using our [theming tools](/docs/faststore/themes-overview) to achieve the desired behavior without compromising the existing integration implemented in `@faststore/core`. You can find the available props for each component on the [Components UI](/docs/faststore/components-index).
...
1. Choose the native section to be customized from the [list of available native sections](/docs/faststore/reference-guides-list-of-native-sections).
...
>⚠️ Change the value of the `SECTION` variable to the name of the section you want to override. In the given example, we set the `SECTION` variable to `ProductDetails` to override this specific section.
...
1. Choose a component to override from the [list of overridable components for each native section](/docs/faststore/reference-guides-list-of-native-sections). In this example, we are overriding the `Price` component.

...
If items are not displaying as gifts, check that the `isGift` property is being returned correctly in your VTEX Order Form and that your GraphQL queries include the `isGift` field on `StoreOffer`.
...
This triggers a WebOps build, and once it completes, the `useIsGiftFromOrderForm` flag will be active in your live store.

...
If items aren't displaying as gifts, check if the `isGift` property is being returned correctly in your VTEX Order Form and that your GraphQL queries include the `isGift` field on `StoreOffer`.

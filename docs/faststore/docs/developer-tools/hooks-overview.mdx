...
FastStore provides a collection of React hooks that give you direct access to built-in store features and state, so you can build customized storefronts faster. This guide covers all available hooks, their categories, implementation patterns, and impact on your store.
...
FastStore hooks are organized by responsibility. Picking the right package for the task makes your code easier to reason about and reduces the risk of unintended side effects:

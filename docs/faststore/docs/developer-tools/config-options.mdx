...
updatedAt: "2025-10-31T13:23:44.263Z"
---
...
>⚠️ As of December 16, 2024, `discovery.config.js` is the new filename for the legacy `faststore.config.js` file. If you're still using the legacy name, see [this release note](https://developers.vtex.com/updates/release-notes/2024-12-16-discovery-config-file) to update it.
...
| [`minPriceAmountFractionDigits`](https://developers.vtex.com/docs/guides/faststore/seo-configuring-product-price-meta-tag-on-pdp) | `number` | Minimum fraction digits used in the PDP `product:price:amount` meta tag. | `2` |
...
|&nbsp;&nbsp;├─ `salesChannel` | `string` | Trade policy that handles catalog, prices, and fulfillment options (e.g.: different regions or seller-specific rules).      | `"1"` |

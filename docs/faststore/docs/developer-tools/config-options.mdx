...
This guide outlines these configuration options for tailoring your FastStore website using the `discovery.config.js` file.
## Example file
...
|-----------------|--------|-----------------------------------------------------------------------------|---------------|
| `title`         | `string` | Store headline displayed in search results (60 chars recommended).      | "FastStore Starter" |
| `description`   | `string` | Brief description of the store (150-160 chars optimal).                | "FastStore Demo Store" |
...
Contains settings related to the store's APIs, which link your FastStore storefront to backend services and manage environmental behavior.
...
| `locale`           | `string`   | Language or locale setting for all store text, date, and numbers. Use standard locale format like `en-US` for English (US), `pt-BR` for Brazilian Portuguese, etc.                                     | "en-US" |
| `channel`          | `object`   | Identifies the commercial and regional context for handling catalog, pricing, and logistics.        |             |
|&nbsp;&nbsp;├─ `salesChannel` | Trade policy that handles catalog, prices, and fulfillment options (example: different regions or seller-specific rules).      | "1" |
|&nbsp;&nbsp;├─ `regionId` | Internal identifier used to define regional segmentation based on the user’s location for services such as catalog, shipping, pickup, and other logistics-related processes. |               |
...
| `addressType`      | `string`   | Address format for fulfilment        | "residential" |
...
| &nbsp;&nbsp;├─ `pdp`       | `string`   | URL of a product details page.                  | "/headphone-white-10000006/p" |
...
| Property                 | Type     | Description                                     | Example |
|---------------------------|----------|-------------------------------------------------|--------------|
| `pages`                   | `object` | Maps the URLs of pages for end-to-end testing with Cypress, with each field defined as a string. | `{ home: "/", pdp: "/headphone-white-10000006/p", collection: "/headphones", collection_2: "/audio", collection_filtered: "/headphones?priceRange=100-200", search: "/s?q=headphone" }`
| &nbsp;&nbsp;├─ `home`               | `string` | URL of the homepage.                           | `"/"` |
| &nbsp;&nbsp;├─ `pdp`                | `string` | URL of the product details page.                | `"/headphone-white-10000006/p"` |
| &nbsp;&nbsp;├─ `collection`         | `string` | URL of a collection page.                       | `"/headphones"` |
...
| `graphqlCacheControl`       | `object` | Settings for GraphQL responses. | `{ "maxAge": 300, "staleWhileRevalidate": 60 }` |
...
| Property         | Type     | Description                                               | Example |
|------------------|----------|-----------------------------------------------------------|---------|
| `webhookUrls`    | `object` | An object of strings that defines the webhook URLs used to interact with the Headless CMS. | ["https://mystore.myvtex.com/cms-releases/webhook-releases"]

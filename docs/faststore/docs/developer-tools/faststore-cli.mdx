...
> ⚠️ Running commands via `yarn` is sufficient for developing your store. However, to set up your project with the Headless CMS and create new sections, you must install the FastStore CLI globally and use its commands.
...
| [`generate`](#generate) | Optimizes the schema and regenerates TypeScript types so that recent changes take effect without restarting the development server. |

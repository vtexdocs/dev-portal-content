...
> ⚠️ Running the commands through `yarn` is enough to develop part of your store. However, to set up your project with Headless CMS and create new sections, you need to install the FastStore CLI globally and use its commands.
...
| [`dev`](#dev) | Starts the project in the development environment and uses `http://localhost:3000` as the default server. It allows you to change the project and see the results locally on your machine in real-time without affecting the production environment. |
...
Starts the project in the development environment and uses `http://localhost:3000` as the default server. This allows you to change the project and see the results locally on your machine in real-time without affecting the production environment.

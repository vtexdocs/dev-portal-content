...
- Ensure you are using `@faststore/cli` version `2.2.9` or above. If you have versions lower than these requirements, update them following the instructions in [Updating the '@faststore/cli' package version](https://developers.vtex.com/docs/guides/faststore/developer-tools-updating-the-cli-package-version).
---
...
Some projects may prefer exclusive execution of their custom Cypress tests. Although it is not our recommendation, for projects using Cypress version 12.x or higher, there is the possibility of running the **Integration Tests** check only for your custom tests.

...
Secrets are sensitive information, such as passwords, tokens, and environment variables, that must be securely managed during the FastStore project deployment process. [WebOps](https://developers.vtex.com/docs/guides/faststore/1-onboarding-overview) now centralizes and standardizes secrets management, ensuring a consistent and secure process across all deployment providers.
...
>ℹ For local setups, use `vtex.env` and add this file to `.gitignore` to prevent version control from tracking it.

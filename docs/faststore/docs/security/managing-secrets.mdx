...
Secrets employs a structured method for interacting with different deployment providers based on project-specific configurations. See below an example of a successful flow that illustrates how WebOps determines the appropriate provider to use depending on the project’s settings and then delegates the build and deployment process accordingly.
...
When a user initiates a request to create or delete a secret (1), WebOps first validates the user's permissions and the request scope (2). The scope defines the context, such as project, environment, namespace, or account, that determines where and how the secret will be stored, propagated, and accessed. Once validated, WebOps forwards the operation to AWS Secrets Manager (3), which confirms the action (4). Next, WebOps identifies the appropriate deployment provider based on the project’s configuration (5). It then securely transmits the secret information to the selected provider (6, 11, 16). The chosen provider consumes the secret and prepares the environment (7, 12, 17), after which WebOps triggers the build and deployment process (8, 13, 18). The provider proceeds to build and host the application (9, 14, 19) and returns a status update to WebOps (10, 15, 20). Finally, WebOps notifies the user that both the secret operation and the deployment were successful (21).
...
In the **Secrets** section, you can [create](#creating-secrets), [update](#updating-secrets), or [delete](#deleting-secrets) following the steps below.

<Tabs items={[Creating secrets, Updating secrets, Deleting secrets]} defaultIndex="0">
  <Tab>

...
WebOps uses [AWS Secrets Manager](https://aws.amazon.com/pt/secrets-manager/) to securely store and manage secrets in a centralized location. This ensures that sensitive information is kept out of your project’s codebase and is retrieved securely by WebOps during deployment.

>ℹ️ In FastStore v1, secrets management required an empty `vtex.env` file at the project's root. Secrets were defined in `secrets.revealed.json` and then encrypted into `secrets.hidden.json` before being committed to the main branch. In FastStore v2+, this workflow has been deprecated - secrets are now managed directly through the WebOps interface.

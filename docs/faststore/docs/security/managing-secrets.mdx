...
>⚠ After creating or updating a secret, there is a short propagation delay before the new values become available to the next build. The secrets are synchronized asynchronously across clusters, which means:
>
> * If you trigger a deploy **immediately** after saving a secret, the build may finish successfully **without** using the updated value.
> 
> * To ensure the new secret value is applied, **wait 3–5 minutes** after saving the secret and then trigger a new deploy.
>
> If you have already deployed right after creating or updating the secret and do not see the new value in your application, wait a few minutes and **run another deploy** before opening a support ticket.

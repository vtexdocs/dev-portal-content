...
>⚠ After creating or updating a secret, there is a short propagation delay before the new values become available to the next build. To ensure the created or updated secret is applied, wait **3-5 minutes** after saving it and then trigger a new deploy.
>
> If you have already deployed right after creating or updating a secret and do not see the new value in your application, wait a few minutes and **run another deploy** before opening a support ticket.

...
Secrets are sensitive information, such as API keys, passwords, tokens, and environment variables, that must be securely managed during the FastStore project deployment. 

[WebOps](https://developers.vtex.com/docs/guides/faststore/1-onboarding-overview) centralizes and standardizes secrets management, ensuring a consistent and secure process across all deployment providers.


WebOps uses [AWS Secrets Manager](https://aws.amazon.com/pt/secrets-manager/) to securely store and manage secrets in a centralized location. This ensures that sensitive information, such as API keys and environment variables, is kept out of your project’s codebase and is retrieved securely by WebOps during deployment.

>ℹ️ In FastStore vX.X, secrets management required an empty `vtex.env` file at the project's root. Secrets were defined in `secrets.revealed.json`, then encrypted into `secrets.hidden.json` before being committed to the main branch. In FastStore vX.X, this workflow has been deprecated - secrets are now managed directly through the WebOps interface.
...
In this guide, learn how to manage secrets in your FastStore project using WebOps.

## WebOps environment

In WebOps, secrets are stored securely using AWS Secrets Manager and are automatically injected into the deployment process by WebOps. This ensures a secure, centralized, and consistent workflow across all supported cloud providers, without exposing secrets in your project code.
## Secrets flow

The Secrets feature employs a structured method for interacting with deployment providers based on project-specific configurations. Below is an example of a flow that illustrates how WebOps determines the appropriate provider based on the project’s settings and then delegates the build and deployment process accordingly.

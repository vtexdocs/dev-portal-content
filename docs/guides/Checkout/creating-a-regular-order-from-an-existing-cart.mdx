...
Use the obtained `orderFormId` to place a new order via the [Place order from an existing cart](https://developers.vtex.com/docs/api-reference/checkout-api#post-/api/checkout/pub/orderForm/-orderFormId-/transaction) endpoint, providing the following information in the request body:

- `referenceId`: unique identifier (ID) of the `orderForm` associated with the shopping cart from which the order will be created (`orderFormId`).
...
Otherwise, if you face errors during the request submission, refer to [Handling HTTP response errors](#handling-http-response-errors) for common error scenarios when placing an order.

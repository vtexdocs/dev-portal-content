...
Send a `GET` request to the [Get current or create a new cart](https://developers.vtex.com/docs/api-reference/checkout-api#get-/api/checkout/pub/orderForm) endpoint to retrieve the `orderFormId` of the shopping cart you intend to use to create an order. The `orderFormId` is a unique identifier of the [orderForm](https://developers.vtex.com/docs/guides/orderform-fields) linked to a specific shopping cart.

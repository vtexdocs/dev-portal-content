...
In addition to valid credentials, make sure that the cart you will use to create the order already contains all the required information. Without these details, the steps described in this guide cannot be completed:
...
> ✅ Click on the steps below to navigate through this article and explore specific topics. Relevant code for each phase or parameter is highlighted on the right side of the screen.
...
> If the payment is processed without any issues, the order should be successfully placed (`204 No Content` status). Otherwise, a `500 Internal Server Error` error might occur.

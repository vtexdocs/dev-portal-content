...
The `defineExtensions` function registers one or more React components by associating them with extension points. More information in [Exploring Sales App extensions](https://developers.vtex.com/docs/guides/exploring-sales-app-extensions).
...
| `addGiftCard` | `(redemptionCodeOrGiftCard: string \| GiftCard, provider?: string) => Promise<void>` | Adds a gift card to the cart payment data. You can pass either<ul><li>a redemption code (`string`) and optionally a `provider`, or</li><li>a [GiftCard object](#giftcard), when you already have the gift card data.</li></ul> |

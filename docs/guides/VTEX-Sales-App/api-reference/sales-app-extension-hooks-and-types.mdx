...
| `addGiftCard` | `(redemptionCodeOrGiftCard: string \| GiftCard, provider?: string) => Promise<void>` | Adds a gift card to the cart payment data. You can pass either<br><br>- a redemption code (`string`) and optionally a `provider`, or<br>- a GiftCard object, when you already have the gift card data. |

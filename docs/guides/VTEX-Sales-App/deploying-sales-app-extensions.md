...
After creating extensions for Sales App, publish them during deployment to make them publicly available.
...
> ⚠️ Since Sales App extensions only support production deployments, to revert a change, create a new commit that reverses the previous one, then push it to the same branch.
...
> ℹ️ Deploying Sales App extensions with WebOps doesn't require the `discovery` module. The storefront can use a different technology, as long as the account is configured to use the `sales-app` module.

## Handling build failures

If the build fails, the issue is usually in the extension code or configuration. Common causes include type-checking errors and typos.

...
The shipping method facet appears only when `showShippingMethodFacet` is enabled. The options listed match `availableShippingValues` when set, or fall back to the default when the prop is not provided. Other Delivery Promise-related facets continue to behave as usual.

...
The main account has access to more modules than the sellers, sharing only some of them, such as [Order Management System (OMS)](https://developers.vtex.com/docs/guides/orders-overview) and [Catalog](https://developers.vtex.com/docs/guides/catalog-overview), as you can see in the diagram below.
...
This is a very common architecture model that integrates physical stores with online stores as [franchise accounts](https://help.vtex.com/en/tutorial/what-is-a-franchise-account--kWQC6RkFSCUFGgY5gSjdl) on VTEX.
...
- **Logistics and OMS**: Each franchise account has its own [logistics](https://developers.vtex.com/docs/guides/fulfillment) settings and does its own [order management](https://developers.vtex.com/docs/guides/orders-overview).
- **Prices and payments**: Each franchise can have its own [pricing](https://developers.vtex.com/docs/guides/pricing-overview) and [payment](https://developers.vtex.com/docs/guides/payments-overview) methods, or it can inherit them from the main account.
...
- **Customer data storage**: Customer data is segregated by marketplace account. Customer information isn't stored within the main account's [Master Data](https://developers.vtex.com/docs/guides/master-data).
...
- **Separate VTEX account**: Each store operates with its own VTEX account.
- **Separate website**: Each store has its own website and can customize a different frontend style.
- **Customer data storage**: Customer data is segregated by account. Customer information isn't shared between accounts, as each has its own [Master Data](https://developers.vtex.com/docs/guides/master-data).
...
- **Catalog, Pricing, and Logistics**: Each store manages these [VTEX core services](https://developers.vtex.com/docs/guides/getting-started#vtex-core-services) independently. Integrations aren't shared between accounts.

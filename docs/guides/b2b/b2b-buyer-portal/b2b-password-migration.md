...
Before setting up password migration, make sure you are familiar with the [B2B user provisioning](https://developers.vtex.com/docs/guides/b2b-user-provisioning) flow, since provisioning users with a legacy password follows the same process with one additional parameter.
...
A-->>L: Do not save password / Password Reset Flow
L-->>User: User Authenticated
...
From this point on, all authentication for migrated users happens entirely within VTEX, and the middleware is never called again for that user.
...
2. [Configure password migration in VTEX](#step-2---configure-password-migration-in-vtex): Register the middleware endpoint and HMAC credentials in your VTEX account.  
...
Any other HTTP status code is treated by VTEX as a technical error, which returns a generic error to the shopper and triggers controlled retry logic on the server side.
...
* `SHA256_OF_BODY` is the base64-encoded SHA-256 digest of the raw request body bytes as received. Do not parse or re-serialize the JSON.
...
Enforce an internal timeout for the legacy validation call (recommended: 2.0–2.5s). If the legacy system does not respond in time, return a technical error (any status that is not 200, 401 or 403).
...
### Step 2 \- Configure password migration in VTEX
...
Use `PUT` [Upsert password migration configuration](https://developers.vtex.com/docs/api-reference/vtex-id-api#put-/api/authenticator/v1/tenants/features/-featureId-) to register your middleware's URL in VTEX and enable legacy credential routing for your account.
...
1. VTEX validates the plaintext password against the current VTEX password policy.  
...
3. If the policy fails, VTEX does not store the weak password. The user is directed to define a new compliant password via the Password Reset Flow. `isLegacyPassword` remains `true` until a compliant password is set. The user is still authenticated in this session.

If the user is already promoted (`isLegacyPassword=false`), VTEX does not call the legacy endpoint again and uses standard VTEX authentication. If multiple concurrent login attempts occur, the system converges to a single promoted state without corrupting credentials.

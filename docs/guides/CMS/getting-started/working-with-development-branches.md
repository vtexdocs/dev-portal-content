...
To work with a Development Branch, you need the [VTEX CLI](https://developers.vtex.com/docs/guides/vtex-io-documentation-vtex-io-cli-installation-and-command-reference) installed, the [Content plugin](https://developers.vtex.com/docs/guides/content-plugin) (`@vtex/cli-plugin-content`) installed, and your account logged in. Also, your [user role](https://help.vtex.com/docs/tutorials/creating-roles) must have the following associated with it:
...
> ⚠️ Development Branches can't be merged into `main`, so content built on a test schema can never accidentally reach the live store. Also, this type of branch is automatically deleted 15 days after it is created.

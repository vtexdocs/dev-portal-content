...
excerpt: "Learn how the pre-analysis anti-fraud flow adds a risk check before authorization for debit card transactions, and how to request its activation."
---

To provide more protection in transactions processed by debit cards, you can add an initial pre-analysis anti-fraud step to the standard payment transaction flow.

In the standard flow, the anti-fraud analysis runs only after the issuing bank authorizes the transaction. With pre-analysis enabled, the anti-fraud provider is also called before authorization, so transactions with initial inconsistencies are stopped before reaching the acquirer.
...
1. **Analyzing initial risk (pre-analysis):** The flow starts with a pre-analysis performed by the anti-fraud system using the data sent by the payment gateway. This step is exclusive to the pre-analysis flow.
...
3. **Authorized:** The acquirer or gateway sends the transaction information to the issuing bank, which responds whether to authorize the transaction. If the bank denies it, the payment is **canceled**. If the bank authorizes it, the status changes to **Authorized**.
...
| --- | --- | --- |
| Pre-analysis (step 1) | The anti-fraud provider finds initial inconsistencies. | The transaction is canceled before reaching the acquirer, so no authorization is attempted. |
| Authorization (step 3) | The issuing bank denies the transaction. | The payment is canceled. |

...
- **Native gift card provider:** set up the VTEX gift card as a payment method. Gift cards are created and managed in the VTEX Admin.
...
> ℹ️ Stores can activate multiple gift card providers simultaneously — for example, the native provider and two external providers.

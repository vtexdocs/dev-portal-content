...
A purchase flow (also known as shopping flow) is the sequence of steps a customer takes to complete a purchase. In VTEX, the available purchase flows are:
...
The Transparent purchase flow is used when customers complete payment information in the standard SmartCheckout template and complete the purchase without leaving the store. This is the most common option to provide a seamless payment experience.

The sequence of events in a Transparent payment flow works as follows:

1. The customer completes payment information in the SmartCheckout template and clicks **Finish Payment**.
...
The Redirect purchase flow is used when customers confirm the payment option in the store and are redirected to a different page to enter payment information and complete the purchase.

This option is used when the partner needs a specific interaction with the customer that isn't provided by the standard SmartCheckout template.

Some examples include: displaying a QR code for payment, completing an application form, and logging in to an external website.
...
The sequence of events in a Redirect payment flow works as described in the following steps:
...
3. The gateway calls the provider with an [Authorization request](https://developers.vtex.com/docs/api-reference/payment-provider-protocol#post-/payments) (POST `/payments`).
4. The provider returns the status `undefined` and the `paymentURL` in the response body of the Authorization request.
...
7. The provider captures customer interaction input.
8. The provider [responds](https://help.vtex.com/en/tutorial/payment-provider-protocol--RdsT2spdq80MMwwOeEq0m#payment-authorization) to `callbackUrl` with the final authorization status (`approved` or `denied`).
9. The provider redirects the customer to the `returnURL` webpage.
...
The Payment App purchase flow is a flexible option that allows customers to go through a custom payment experience without leaving the store. It supports a wide variety of payment methods through its interaction with the Checkout API. Unlike the Transparent and Redirect flows, Payment Apps must be developed and deployed using [VTEX IO](https://developers.vtex.com/docs/guides/vtex-io-documentation-what-is-vtex-io).

The sequence of events in a Payment App flow works as follows:
...
When a customer completes a purchase, a new payment request is created, starting with the Authorization operation.
...
The payload is large and includes information such as cart items, billing address, order ID, payment methods, and `callbackUrl`. For more information about the endpoints and their payloads, see the [Implementing a Payment Provider](https://developers.vtex.com/docs/guides/payments-integration-implementing-a-payment-provider) article.

The provider then responds to the request. If the payment can't be processed right away, the transaction status changes to `undefined`. This doesn't mean the payment was canceled or denied; it's still being evaluated.
...
While the Contingency Mode is active, all transactions that support asynchronous flow are retained. During this period, the transaction remains in `authorizing` status.

This process takes an average of two hours and can hold up to 2,000 requests. When the provider's system returns to stability, the Contingency Mode status is deactivated, and the system reattempts processing for the transactions on standby.
...
When a payment is approved and already settled but canceled after that, VTEX makes the [POST Refund Payment](https://developers.vtex.com/docs/api-reference/payment-provider-protocol#post-/payments/-paymentId-/refunds) call to the payment provider. Also, the Payment Provider Protocol accepts partial refunds when necessary.

Regardless of the provider's response, VTEX ends the transaction with the status `canceled`.

#### Payment approved but not settled

When a payment is approved but canceled before settlement, VTEX makes the [POST Cancel Payment](https://developers.vtex.com/docs/api-reference/payment-provider-protocol#post-/payments/-paymentId-/cancellations) call to the payment provider.

Regardless of the provider's response, VTEX ends the transaction with the status `canceled`.

#### Payment not approved

VTEX sends the [POST Cancel Payment](https://developers.vtex.com/docs/api-reference/payment-provider-protocol#post-/payments/-paymentId-/cancellations) request to the payment provider once there is no need to refund a payment that wasn't settled.

If the provider doesn't authorize the payment cancellation and returns an `undefined` status, the flow runs asynchronously for a day. If the provider doesn't respond within this period, the payment is canceled.

### Settlement

Once the Gateway authorizes the payment, VTEX sends the [POST Settle Payment](https://developers.vtex.com/docs/api-reference/payment-provider-protocol#post-/payments/-paymentId-/settlements) request to the provider. If the provider doesn't authorize the settlement right away, the request continues to run asynchronously for a day.
...
Since the selection of the payment method depends on the customer, **your provider must be prepared to receive multiple, repeated requests** at those endpoints.

...
hidden: false
createdAt: "2024-06-13T14:58:51.217Z"
updatedAt: "2024-06-13T14:58:51.217Z"
...
Alongside you can see the [`HeaderComponentProps`](focus://3:5) interface, which specifies the types of the props `title` and `subtitle`. This interface is used to type-check the props that are passed to the `HeaderComponent`.

The [`HeaderComponent`](focus://8:15) is a functional component that accepts `title` and `subtitle` as props, which are destructured from the props objet defined by the interface.
...
```js ImageComponent.tsx mark=11:12,17:36
import React from 'react';
...
Consider a component that allows users to input an initial and a final date (e.g., `DateComponentProps.tsx`).

In the `schema`, the `initialDate` and `finalDate` props define the configuration for date input fields.

<CH.Code>

```js	DateComponent.tsx mark=11:12,17:37
import React from 'react';
...
Consider a [component](focus://7:15) that allows users to toggle its active or inactive state. To allow users to activate or deactivate the component, define the [`schema`](focus://17:27) as follows.
...
Consider a [component](focus://CustomComponent) that renders an image, taking an `image` prop, which includes the URL, alt text, and title.

The [`schema`](focus://schema) below defines the structure of the component, specifying that it expects an `image` prop, which is an object containing `url, `alt`, and `title` props.
...
Consider a [component](focus://3:19) where the displayed images adapt to the screen size.
...
In the [`schema`](focus://), the `images` prop is specified as an array of objects, ensuring it will hold multiple items. Each item within the `images` array is defined as an object with three specific props, `src`, `alt`, and `mobileSrc`.
...
```js ArrayComponent.tsx mark=49:53
import React from 'react'
...
There are some props used to define a set of allowed values for a particular prop, such as [`enum`](#enum-example) and [`enumNames`](#enumnames-example).

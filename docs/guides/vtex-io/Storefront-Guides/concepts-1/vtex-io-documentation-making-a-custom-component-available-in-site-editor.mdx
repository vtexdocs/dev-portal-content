...
1. Create a new folder in the `/react` root directory. Follow the `react/components/{ComponentName}/index.tsx` pattern. The folder structure for a component named `CustomComponent` is shown alongside.

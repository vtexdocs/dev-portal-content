...
| `autoFocus`               | `Boolean`                                     | Defines whether the search input should autofocus.                                                                                                                                              | -              |
| `blurOnSubmit`            | `Boolean`                                     | Defines whether the input should blur on submit.                                                                                                                                                          | `false`        |
...
| `emptyPlaceholder`        | `String!`                                     | Shows a placeholder when the ResultList has no results to display.                                                                                                                               | -              |
| `hasIconLeft`             | `Boolean`                                     | Defines whether the search icon is on the left or right.                                                                                                                                          | -              |
...
| `inputType` | `enum` | Defines the value for the `type` HTML attribute (from the `<input>` field). Possible values are: `search` and `text`. We strongly recommend that you use `search` as it fixes some iOS issues. The `text` value is only kept as default for backwards compatibility. | `text` |
| `maxWidth`                | `Number` \| `String`                          | Max width of the search bar                                                                                                                                                                       | -              |
| `minSearchTermLength`     | `Number`                                      | If defined, it will block searches where the term length is less than `minSearchTermLength`.                                                                                                    | -              |

...
> ⚠️ Scripts used by Checkout are linked to the Checkout UI Settings version that's installed in your store. If a prior app version was already installed and you want to change the scripts linked to it, you'll need to repeat the already existing code and thereafter launch the app's new version containing the changes you just made. Housekeeper is responsible for updating app versions in the accounts it's installed.

> > ⚠️ If you are using the Legacy CMS Portal: You can create a workspace and install the app there. While using the app, you will need to apply your changes and copy/paste the content of the files to your portal.
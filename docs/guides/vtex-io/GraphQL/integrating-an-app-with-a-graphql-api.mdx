...
For your app to have permission to access any external resource, you must declare the related [policies](https://developers.vtex.com/docs/guides/vtex-io-documentation-policies) in your app's `manifest.json` file. In the case of a GraphQL API from another app, this is a role-based policy, so you need to add the app and policy names. You can find the policy name and description in the `policies.json` file from the app's source code.

...
> ℹ️ If you want to use mutations, use the function `this.graphql.mutate` and insert the template string as the `mutate` argument's value.
...
For your app to have permission to access any external resource, you must declare the related [policies](https://developers.vtex.com/docs/guides/vtex-io-documentation-policies) in your app's `manifest.json` file. In the case of a GraphQL API from another app, this is a role-based policy, so you must add the app name and policy name. You can find the policy name and description in the `policies.json` file from the app's source code.
...
To test your app, [link](https://developers.vtex.com/docs/guides/vtex-io-documentation-linking-an-app) it to the VTEX platform to enable it to run and accept requests.

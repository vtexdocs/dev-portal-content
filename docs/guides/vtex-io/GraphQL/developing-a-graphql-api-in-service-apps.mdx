...
> ℹ️ Your app may already have the GraphQL builder in the schema. If that's the case, you only have to check if it uses the latest builder version (1.x).
...
2. In the new folder, add the TypeScript files that will have resolver code. We recommend creating a `.ts` file for each resolver.
3. In the created files, add the resolver code. The following example shows a resolver that you can use as reference:
...
With your app running, you can test the implemented endpoints by making requests. One way to do this is using the VTEX [GraphQL IDE](https://developers.vtex.com/docs/guides/graphql-ide) in the Admin:

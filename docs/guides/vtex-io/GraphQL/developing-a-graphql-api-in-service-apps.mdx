...
This guide will show you how to make an API using GraphQL and VTEX IO.

> ℹ️ For details on REST API development, see [Developing services in VTEX IO](https://developers.vtex.com/docs/guides/developing-services-on-vtex-io).
...
- Knowledge of GraphQL and its concepts, such as [queries](https://graphql.org/learn/queries/), [schema](https://graphql.org/learn/schema/), and [resolvers](https://graphql.org/learn/execution/).
...
3. The resolver uses a [VTEX client](https://developers.vtex.com/docs/guides/vtex-io-documentation-clients) to call the desired resource ([VTEX commerce API](https://developers.vtex.com/docs/guides/getting-started-list-of-rest-apis), another app, external service, etc.).
...
Open the `manifest.json` file in your app root and add the [GraphQL builder](https://developers.vtex.com/docs/guides/vtex-io-documentation-graphql-builder). The builder is responsible for interpreting the GraphQL schema in your TypeScript code in the `node` folder.

> ℹ️ Your app may already have the GraphQL builder in the schema. If that is the case, you only have to verify if it uses the latest builder version (1.x).
...
In the `graphql` folder, add the `.graphql` files to define the GraphQL schema. You can add the whole schema in a single file or use as many files as you want, including subfolders. To better organize your project, we recommend using the following structure:
...
Your app will likely access an external resource such as [VTEX Core commerce APIs](https://developers.vtex.com/docs/guides/getting-started-list-of-rest-apis), an endpoint from another app, or a service external to VTEX. In VTEX services, these interactions are handled through [clients](https://developers.vtex.com/docs/guides/vtex-io-documentation-clients).
...
To access routes and resources not present in the native clients, you need to [develop your custom client](https://developers.vtex.com/docs/guides/vtex-io-documentation-how-to-create-and-use-clients). Just like with native clients, you must register each custom client in the `Clients` class located in `node/clients/index.ts` so it can be accessed across your app.
...
2. In the new folder, add the TypeScript files that will have the resolvers' code. We recommend creating a `.ts` file for each resolver.
3. In the created files, add the resolvers’ code. The following example shows a resolver example that you can use as a reference:
...
After defining your resolver functions, add them to the `Service` class of the `node/index.ts` file. You should add a `graphql` field with the resolvers for each endpoint implemented with the directives.
...
With your app running, you can test the implemented endpoints by making requests. One way to do it is using the VTEX [GraphQL IDE](https://developers.vtex.com/docs/guides/graphql-ide) in the Admin with the following steps:
...
4. Select the VTEX IO app you linked.

...
// Type of the Client, used to connect with external APIs
export default class GithubClient extends ExternalClient {
  // Routes map (not mandatory to do it like this)
  private routes = {
  ...
  // Configuration options to each request of this Client. It extends AxiosRequestConfig adding some IO-specific options
  constructor(context: IOContext, options?: InstanceOptions) {
    super('https://api.github.com', context, {
      ...options,
      retries: 2,
  ...
  public getRepo({ owner, repo }: GitRepo, token?: string) {
    return this.http.get<RestEndpointMethodTypes['repos']['get']['response']>(
      this.routes.repo({ owner, repo }),
      {
        // The metric parameter gives a name to identify each HTTP call
        metric: 'git-repo-get',
        headers: {
          Accept: 'application/vnd.github.nebula-preview+json',
          ...(token && { Authorization: `token ${token}` }),
  ...
  public async getUser({ username }: { username: string }) {
    // Using getRaw method is useful for  retrieving other info about the response, such as headers. The get method retrieves just the body.

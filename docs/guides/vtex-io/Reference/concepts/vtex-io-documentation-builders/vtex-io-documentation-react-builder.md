...
The basic structure of the `/react` folder consists of the following files, as you can see in the [React boilerplate app](https://github.com/vtex-apps/react-app-template/tree/master/react):
...
- `package.json`: Defines the project's metadata, dependencies, custom scripts that use [Yarn](https://yarnpkg.com/migration/overview), and other configuration settings.
...
>ℹ️ Learn how to link a React component to a Store Framework block in [the Store builder](https://developers.vtex.com/docs/guides/vtex-io-documentation-store-builder).

## Use case examples

The `react` builder is versatile and enables the development of frontend apps with different characteristics and objectives. The [default template](#folder-structure) provides only the essential files, but the possibilities are countless.
...
>ℹ️ An app using the `react` builder can export one or more components.

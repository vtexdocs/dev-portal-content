...
After defining your theme block, it is time to think about the style you want for it — setting colors, typography, and other elements that establish the app's visual identity when rendered.

To define the style of your components, we will use *Cascading Style Sheets* - **CSS**.
...
- **CSS Modules** - Secondary option for styling components from CSS classes. This option should only be used when the desired style cannot be configured using Tachyons.
- **CSS Handles** - Exports generic CSS classes to allow users that work with your component to customize it for their respective scenarios.

Now that you know the difference between the tools available on the platform, you are ready to learn more about them.

Below, you can find the instructions for each of these tools and their purpose.

> ℹ️ Customizing the style in a component requires basic CSS knowledge. We recommended checking the [documentation](https://www.w3schools.com/css/default.asp) for this language before following the instructions detailed in this article.
...
In practice, VTEX Tachyons helps you use CSS classes in your storefront app with little effort.

> ℹ️ To better understand VTEX Tachyons tokens for CSS classes, it is important to familiarize yourself with the Tachyons solution. Carefully read its [documentation](https://tachyons.io/#getting-started).
...
CSS Modules is a CSS file in which CSS classes are designed based on the storefront app that exports the component.
> ⚠️ By defining CSS classes this way, the CSS Modules tool becomes limiting when considering importing and exporting components between different apps. That is why we recommend only using VTEX Tachyons in your project. **Use CSS Modules only for cases that cannot be solved by Tachyons.**
...
3. Still from the `react` folder, access the component file being customized and import the `styles` file created in step 1. For example:
...
6. Save the changes your changes and [link](https://developers.vtex.com/docs/guides/vtex-io-documentation-linking-an-app/) the app.
...
To make handles available for future app users, add `vtex.css-handles` to the `dependency` list in the `manifest.json` file:
...
However, when generating handles, you must pay attention to how the React components of your app are imported and defined.
...
3. In the `CSS_HANDLES` array, replace the example strings with new ones (that have the names of the *CSS handles* you want). You can choose any name for the *CSS handle*; but, remember, your app users will use it with CSS classes to personalize HTML elements in stores. We recommend choosing intuitive names.
...
4. Add the new *CSS handle* to the desired CSS class —according to the HTML element to be customized by app users. Remember to use the `handle` variable when adding it, as well as the *CSS Handle* name defined in the `CSS_HANDLES` array (like `container`). For example:
...
3. In the `CSS_HANDLES` *array*, replace the example strings with new ones (that have the names of the *CSS handles* you want). You can choose any name for the *CSS handle*; but, remember, your app users will use it with CSS classes to personalize HTML elements. We recommend using intuitive names.

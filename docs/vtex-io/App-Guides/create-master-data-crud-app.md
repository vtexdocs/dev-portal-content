...
>⚠️ If you are using a development environment, use `vtex setup -i`.
...
>⚠️ You are importing your data entity typings from the app itself. Identify your app in the format `'{vendor}.{name}'`, according to the information in you `manifest.json`. If your app does not recognize the generated typings import, see the solution presented in the [Typings](#typings) section below.
...
> ℹ️ If you run into issues with the data entity typings generation or import, try generating the typings again.
...
> ℹ️ You may use Master Data functions in your app’s `resolvers` or `middlewares`  folder.

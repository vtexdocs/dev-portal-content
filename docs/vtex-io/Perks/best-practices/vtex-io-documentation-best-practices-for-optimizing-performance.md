...
You can deactivate the native service worker provided by VTEX IO to successfully use a third-party solution in your store.
...
Note that every `menu-item` was previously configured as a children/block. With the change, the `menu-items` group declared as props are interpreted as a single block (blue square).
...
The size of the images displayed in your store can directly impact overall performance. To optimize image rendering in your website, we recommend that you adopt the practices detailed in [Best practices for rendering images](https://developers.vtex.com/docs/guides/vtex-io-documentation-best-practices-for-rendering-images/) regarding the media type used in your theme.

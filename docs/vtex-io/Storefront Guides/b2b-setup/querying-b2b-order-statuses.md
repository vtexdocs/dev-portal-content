...
| ↳ `tipo`            | integer  | Type of code that will be used in the query.<br></br><br></br>If you want to filter the query using `PedidoId` (web order number, which is the identification number of the order placed through the store’s website), the value should be `0`.<br></br><br></br>To filter the query using `PedidoClienteId` (identification number associated with the customer who made the purchase), the value should be `1`.<br></br><br></br>To filter the query using `PedidoErpId` (ERP order number), set the value to `2`. |

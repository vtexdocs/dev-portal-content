...
With the Analytics client implemented, we want to use the events fired by the `events-example` app as triggers to the requests to the Analytics client. This means that, for every event listened, we should make a request to the Analytics app. So, every 3 seconds, we will have new data on **Live Products**.

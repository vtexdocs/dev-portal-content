...
With our new analytics client, let's update our event handler to display the output of the analytics client's `getLiveUsers` method. Go to the `node/handlers/analytics.ts` file and replace the content of `ctx.body` with the method developed in the previous steps (`ctx.analytics.getLiveUsers()`).
...
### Relating a route to a event handler

Update the `node/index.ts` file to relate a route to an event handler. Note that the `routes` section associates the `analytics` route with the `analytics` event handler function. The `GET` method is associated with the `analytics` event handler.

Therefore, when a GET request is made to the `analytics` route previously defined, it triggers the `analytics` event handler function, which, in turn, executes the `getLiveUsers()` method of the Analytics client, providing real-time insights into product views.

...
excerpt: "VTEX Intelligent Search now supports negative filters in the facets path using the not: prefix, so you can exclude facet values from results."
---
...
Existing behavior for combining filters is unchanged: multiple values of the same facet type are combined with **OR**, and values of different facet types with **AND**. The NOT operator applies on top of that model to exclude a given facet value.
...
No change is required for existing integrations. URLs that do not use `not:` behave as before.

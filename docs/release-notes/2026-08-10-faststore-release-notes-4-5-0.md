...
FastStore 4.5.0 expands B2B My Account with a quotes list and contract switcher, adds CMS-configurable product recommendations and custom account pages, and improves multi-locale PDP/PLP URL resolution. It also forwards Intelligent Search Pricing Fallback tokens into Checkout and copies self-hosted fonts from `public/` during CLI generate. See the sections below for details.
...
> ⚠️ **Breaking change (behavioral):** `StoreCollection.type` no longer returns `Cluster` or `SubCategory` (clusters report as `Collection`; deeper categories as `Category`). Unmatched single-segment slugs that previously fell back to full-text search now return 404. Stores that branch on `collection { type }` or relied on search fallback should review those paths after upgrading. Enum values remain declared for compatibility; cleanup is tracked for a future major.
...
Adds a CMS-configurable `RecommendationShelf` section that fetches VTEX personalization recommendations for a campaign VRN and renders them with the shared core `ProductCard`. The anonymous personalization session now starts via a CMS Global Section rather than a discovery config flag: shelves still fetch when placed on a page, but the session only starts when that global section is present.

After upgrading to `v4.5.0`, add the **Recommendation Shelf** section in the CMS and on the desired page with a valid campaign VRN.
...
Stores that place fonts or nested assets in `public/` should upgrade `@faststore/cli` to `v4.5.0` and re-run generate/build so files copy into `.faststore/public/`. No configuration changes are required.
...
Accounts with the Intelligent Search Pricing Fallback flag enabled should upgrade to `v4.5.0`. No FastStore config change is required; ensure the platform Pricing Fallback flag is on for the account.
...
Stores with FastStore My Account and the Quotes module enabled should upgrade to `v4.5.0` to surface the Quotes page for eligible B2B members. No extra storefront configuration is required beyond the existing My Account enablement and Quotes platform setup.
...
Upgrade to `v4.5.0` on B2B stores using FastStore My Account. No discovery config flag is required for production; local development may need authenticator/vtexid rewrites as documented in the PR for non-WebOps environments.

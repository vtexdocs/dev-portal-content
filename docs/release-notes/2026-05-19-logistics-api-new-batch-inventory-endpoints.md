---
title: "New batch inventory endpoints in the Logistics API"
...
- `POST` [Create batch inventory job](https://developers.vtex.com/docs/api-reference/logistics-api#post-/availability/v1/inventory/batch): Creates a new batch inventory job and returns a pre-signed URL for uploading the CSV file. The batch remains in `AWAITING_UPLOAD` status until the CSV is uploaded and committed.
...
The existing inventory endpoints are optimized for low-latency, per-SKU updates, but they are not suited for scenarios that involve millions of SKUs or full inventory synchronizations. The new batch endpoints provide a scalable, asynchronous flow for these use cases, allowing you to:

- Update large volumes of inventory data in a single operation using CSV files.
...
No action is required for existing integrations. The previously available [inventory endpoints](https://developers.vtex.com/docs/api-reference/logistics-api#tag/Inventory) continue to work without changes.

To use the new batch operations endpoints, refer to the [Logistics API reference](https://developers.vtex.com/docs/api-reference/logistics-api) and contact our [Support Center](https://support.vtex.com/hc/en-us/requests) to enable early access.

...
FastStore now route Checkout and My Account pages though the store's primary domain (e.g., `www.domain.com`), removing the need for a separate `secure.domain.com` subdomain. This reduces configuration steps and avoids cross-domain redirects.
...
1. Open a ticket with [VTEX Support](https://help.vte.com/support) requesting the account be configured for a single-domain setup. Please include the following information in the ticket:

...
| --- | --- | --- |
| `--help` | `-h` | Displays help information for the command. |
| `--out=<path>` | `-o` | Specifies the output file path for the generated schema. This option is required to produce an output — without it, the command only displays a success message and doesn't save the schema. |
| `--local=<path>` | `-l` | Uses a local base schema file instead of fetching the latest FastStore schema from a remote source. |
| `--remote=<path>` | `-r` | Specifies a remote URL to fetch a custom base schema instead of the default FastStore schema. The URL must be publicly accessible and return valid JSON. |
| `--beta` | `-b` | Fetches the default base schema from the beta Schema Registry instead of the stable registry. |

...
> ⚠️ If your domain isn't listed, it may indicate an issue with the configuration or DNS settings that need to be addressed before proceeding with the go-live. If you follow the [external DNS settings](https://developers.vtex.com/docs/guides/faststore/go-live-1-configuring-external-dns) guide correctly and the problem persists, open a ticket to [VTEX support](https://help.vtex.com/en/support).
...
In this section, you can update or delete an existing secret by clicking `⋮` and choosing the corresponding option, `Edit` or `Delete`.
...
> ⚠️ Proceed with caution. Deleting the project can't be undone, and all deployments and associated domains will be permanently removed.

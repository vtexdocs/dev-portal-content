...
This builder interprets the `/react` directory, where you must place the app’s React code using [TypeScript](https://www.typescriptlang.org/). When you run your app, this code is transformed into compiled bundles, ready to be used in frontend apps.

To learn how to implement apps using this builder, see the [Storefront apps guide](https://developers.vtex.com/docs/guides/vtex-io-documentation-1-developing-storefront-apps-using-react-and-vtex-io).
...
- `typings`: Folder with TypeScript type definitions.
- `.tsx` files: Contain the source code of React components. Replace `{ReactCodeFileName}` based on your scenario.
- `tsconfig.json`: Specifies how the TypeScript compiler should behave, indicating the root directory and the output directory for the compiled code, as well as other TypeScript project settings. For details about this file, see [`tsconfig.json` documentation](https://www.typescriptlang.org/docs/handbook/tsconfig-json.html).
- `package.json`: Defines the project metadata, dependencies, custom scripts using [Yarn](https://yarnpkg.com/migration/overview), and other configuration settings.
- `yarn.lock`: Locks down the exact versions of your project dependencies, including transitive dependencies, to ensure consistent builds across different machines and environments. This file is automatically generated based on the dependencies in your `package.json` file.
...
2. Modify the `manifest.json` file in the React example app to include your app metadata, such as its name, version, vendor, and dependencies. The `react` builder must also be declared in the app’s `manifest.json` file as follows:
...
- The `components` folder contains the folders for each React component.
- The `graphql` folder contains GraphQL queries and mutations, which some components use to communicate with the backend. Keeping the GraphQL code separate from other parts of the React component promotes modularity and makes it easier to manage queries, mutations, and subscriptions that are specific to that component.

...
- `useGeoLocation`: Interact with the browser’s geolocation API or related client-side mechanisms to attempt to auto-detect the user’s location.

## Default settings

Delivery Promise feature is defined in the `deliveryPromise` section of the [`discovery.config.js`](https://developers.vtex.com/docs/guides/faststore/project-structure-config-options) file.
...
- `mandatory`: When set to **false**, providing a postal code is optional for the user. Customers can browse and add items to cart without submitting their zip code, though the accuracy of delivery information may be limited.
...
Open a ticket to [VTEX support](https://help.vtex.com/en/support) and request the activation of Delivery Promise for your account
...
### Step 4 - [Optional] Defining default postal code
...
3. **Default postal code:** If the user hasn't provided a location, isn’t logged in, or has no saved address, we check if the merchant has configured a default postal code for all buyers. Learn to do so in [Defining a default postal code](#step-4-optional-defining-default-postal-code).

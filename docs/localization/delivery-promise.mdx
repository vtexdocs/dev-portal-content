...
- `mandatory`: When set to **false**, providing a postal code is optional for the user. Customers can browse and add items to the cart without submitting their postal code, though the accuracy of delivery information may be limited.
...
The default postal code serves as a fallback mechanism when the user doesn't provide a location, isn't logged in, or has no saved address.

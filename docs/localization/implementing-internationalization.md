...
A **binding** represents a storefront surface that combines a domain (canonical base address), one or more locales, currency, and the sales channels (trade policies) it serves. This configuration is essential for operating multiple languages within a single VTEX account.
...
1. Access `{accountName}.myvtex.com/admin/binding`. Replace `{accountName}` according to your scenario.  
2. Click the kebab menu (`⋮`) alongside the canonical address you want to configure, then click `Edit`.  
3. Set the canonical base address to your public domain (and the locale path, if applicable).  
4. Go to **Locales** and click `+ Add` to register a new locale.  
...
description="Learn how to disable automatic translation."  
...
description="Learn how to overwrite automatic translation from catalog."  

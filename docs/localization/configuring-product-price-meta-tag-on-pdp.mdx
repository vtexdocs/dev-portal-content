...
FastStore allows you to control how the `product:price:amount` meta tag is formatted on Product Detail Pages (PDPs). Use the `minPriceAmountFractionDigits` setting in [`discovery.config.js`](https://developers.vtex.com/docs/guides/faststore/developer-tools-config-options) to define the minimum number of decimal places displayed in this meta tag. This is useful to ensure consistency between:

- The Open Graph meta tag (`product:price:amount`).
- The structured data included in the PDP HTML (`ProductJsonLd` schema).
- External integrations such as [Google Merchant Center](https://support.google.com/merchants/answer/12159157?hl=en&ref_topic=15302229&sjid=17079996474061819089-SA).

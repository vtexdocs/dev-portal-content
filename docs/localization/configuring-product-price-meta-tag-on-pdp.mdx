...
FastStore allows you to control how the `product:price:amount` meta tag is formatted on Product Detail Pages (PDPs). Use the `minPriceAmountFractionDigits` setting in `discovery.config.js` to define the minimum number of decimal places displayed in this meta tag. This is useful to ensure consistency between:

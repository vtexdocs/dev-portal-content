...
* [Instructions](#instructions): Instructions for enabling and deploying performance optimizations in your store, including workspace setup, Admin configuration, and promoting changes to production.
* [Optimization options](#optimization-options): Methods to enhance store performance, categorized by:
  ...
  * [Manual optimizations](#manual-optimizations): Additional manual actions for advanced performance that require direct code changes.
...
We recommend that stores using `minicart.v2` and `add-to-cart-button` blocks (instead of `minicart` and `buy-button`, respectively) disable the legacy provider. Consuming a single provider will improve store performance.
...
When enabled, the page content is proactively downloaded in the background when you hover over a link. This means that the page is already loaded or partially loaded when you click the link.
...
This script contains significant web page data and can be long, demanding browser processing. When you enable lazy runtime, the script is broken into smaller scripts to prevent compromising the total blocking time of the store website. This helps avoid lengthy tasks, leading to a faster store experience.
...
This action removes the store name from the end of page titles on product and category pages, resulting in shorter titles that may highlight product keywords better for SEO.
...
However, the internal `menu-items` (Clothing, Accessories, and Eyeglasses) can be changed from children to props.
...
This approach reduces the number of blocks from 3 to 1, improving store performance.
...
Build the shelf and carousel blocks of your theme using the [Slider Layout](https://developers.vtex.com/docs/guides/vtex-io-documentation-building-a-carousel-using-slider-layout) component.

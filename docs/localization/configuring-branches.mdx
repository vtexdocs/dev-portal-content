...
- **Include only active development branches:** Configure include patterns (for example, `feature/*`) to generate previews only for feature branches, excluding experimental or abandoned branches.- **Exclude release branches:** Use exclude patterns (for example, `release/*`) to prevent release branches from generating preview builds, as these are typically merged directly to production.
...
> ⚠️ After changing the production branch, you may need to redeploy your store. WebOps will display an alert if a redeploy is required.
...
> ⚠️ After activating branch filtering or adding filter rules, you may need to redeploy your store. WebOps will display an alert if a redeploy is required.
...
> ⚠️ After updating a rule, you may need to redeploy your store. WebOps will display an alert if a redeploy is required.
...
> ⚠️ After deleting a rule, you may need to redeploy your store. WebOps will display an alert if a redeploy is required.